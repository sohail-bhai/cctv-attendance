        ...
        <section style={S.card}>
          ...
          {loadedSource === 'saved' && loadedSession?.source_report_superseded && (
            <div style={merge(S.qualityWarning, { borderColor: '#bfdbfe', background: '#eff6ff', color: '#1d4ed8' })}>
              ...
              <p style={{ margin: '6px 0 0' }}>The earlier frame-only result is preserved as a superseded quality-failed revision. This report uses reviewed tracklet evidence and does not treat insufficient camera evidence as absence.</p>
            </div>
          )}
              ...
              <p style={{ margin: '6px 0 0' }}>
                {loadedSession.attendance_finalized
                  ...
                  : Number(loadedSession.unresolved_count || 0) > 0
                    ? `${loadedSession.unresolved_count} student(s) still require an explicit review decision.`
                    : 'All doubtful cases are resolved. Open Review Students to explicitly finalize this report.'}
              </p>
            </div>
          )}
              ...
              <tbody>
                {filteredRows.map((row) => (
                  <tr key={getRoll(row)} style={rowMatchesReviewMode(row, 'review', {}, {
                    ...
                    <td style={S.td}>{row.Reviewed_Tracklet_Checkpoint_Count ?? row.Recognized_Checkpoints ?? 0}/{row.Total_Checkpoints || cleanSlot?.checkpoints || 5}</td>
                    <td style={S.td}>
                      {row.Reviewed_Tracklet_Checkpoints ? <strong style={{ color: '#0f766e' }}>Multi-frame: {row.Reviewed_Tracklet_Checkpoints}</strong> : `${getDetectionCount(row)} observations`}
