  ...
  const load = async () => {
    ...
    const stateMessage = data.review_state?.attendance_finalized
      ...
      : (data.review_state?.unresolved_count || 0) > 0
        ? `${data.review_state.unresolved_count} student(s) still need review.`
        : 'All doubtful cases are resolved. This report is ready to finalize.';
        ...
        <section style={S.card}>
          ...
          {entry.source_report_superseded && (
            <div style={merge(S.notice, S.noticeInfo, { marginTop: 16 })}>
              <strong>Corrected multi-frame authority revision</strong>
              <div style={{ marginTop: 5, fontWeight: 700 }}>The original frame-only report is preserved as superseded. Present, Needs Review, Unconfirmed, and Missing Enrollment now come from the reviewed authority evidence.</div>
            </div>
          )}
          ...
          <div style={merge(
            ...
            <strong>
              {finalized
                ...
                : summary.reviewCases > 0
                  ? `${summary.reviewCases} student(s) still need review`
            ...
            <div style={{ marginTop: 5, fontWeight: 700 }}>
              {finalized
                ...
                : summary.reviewCases > 0
                  ? 'Choose Present or Absent for every doubtful case, add a reason, then save.'
                  : 'All doubtful cases are resolved. Finalize to create the official reviewed CSV.'}
            </div>
          </div>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {filteredRows.map((row) => {
                  ...
                  const evidence = `${row.Recognized_Checkpoints || 0}/${row.Total_Checkpoints || 5} checkpoints · ${row.Total_Accepted_Detections || row.Detection_Count || 0} observations`;
                  const reviewedTracklets = row.Reviewed_Tracklet_Checkpoints || row.Guarded_Recovery_Candidate_Checkpoints || '';
                  ...
                  return (
                    <tr key={row.Roll_Number} style={needsReview ? { background: '#fffbeb' } : null}>
                      ...
                      <td style={S.td}>
                        ...
                        {reviewedTracklets ? <div style={{ color: '#0f766e', fontSize: 12, marginTop: 4, fontWeight: 850 }}>Multi-frame checkpoints: {reviewedTracklets}</div> : null}
