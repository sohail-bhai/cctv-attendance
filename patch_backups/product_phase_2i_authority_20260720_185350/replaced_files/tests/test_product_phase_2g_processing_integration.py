...
class PolicyTests(unittest.TestCase):
    def test_fixed_policy_uses_validated_compare_modes(self):
        ...
        self.assertEqual(policy["zone_mode"], "compare")
        self.assertEqual(policy["tracklet_mode"], "compare")
        self.assertEqual(policy["official_recognition_authority"], OFFICIAL_RECOGNITION_AUTHORITY)
        self.assertFalse(policy["tracklets_used_for_official_attendance"])
    ...
    def test_client_cannot_lower_threshold_or_checkpoint_rule(self):
        for payload in (
            ...
            {"processing_mode": PROCESSING_MODE, "tracklet_mode": "tracklets"},
            {"processing_mode": PROCESSING_MODE, "zone_mode": "zones"},
        ):
...
class WiringContractTests(unittest.TestCase):
    ...
    def test_browser_command_enables_diagnostics_but_keeps_frame_authority(self):
        policy = fixed_processing_policy({"processing_mode": PROCESSING_MODE})
        command = build_processing_command(
            ...
            policy=policy,
            diagnostic_run_id="product-2g-test",
        )
        joined = " ".join(command)
        self.assertIn("--require-authoritative-roster", command)
        self.assertIn("--diagnostic", command)
        self.assertIn("--zone-mode compare", joined)
        self.assertIn("--tracklet-mode compare", joined)
        self.assertNotIn("--tracklet-mode tracklets", joined)
        self.assertIn("build_processing_command(", self.app_text)
        self.assertIn('"tracklets_used_for_official_attendance": False', self.app_text)

    def test_frontend_no_longer_controls_thresholds(self):
        self.assertIn("processing_mode: 'quality_aware_production_v1'", self.frontend_text)
