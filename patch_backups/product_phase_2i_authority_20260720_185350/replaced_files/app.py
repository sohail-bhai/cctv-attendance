...
def build_effective_processing_options(video_dir: Path, payload: dict | None = None) -> dict:
    """Return the fixed quality-aware Product Phase 2G production policy.
...
def run_slot_job(
    ...
    diagnostic_run_id = clean_session_filename(
        f"product-2g-{session['session_id']}-{job_id.lower()}"
    )
    ...
    try:
        ...
        # The shared Product Phase 2G command builder already enables unbuffered output.
        ...
        if not roster_check.get("roster_complete"):
            raise RuntimeError(
                "Product Phase 2G processor did not produce the authoritative roster: "
