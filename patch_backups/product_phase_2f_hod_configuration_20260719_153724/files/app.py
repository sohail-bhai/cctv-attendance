...
def review_row_requires_attention(row: dict) -> bool:
    ...
    saved_resolution = _truthy(row.get("Manual_Override")) and ("present" in lowered or "absent" in lowered)
    if saved_resolution:
        return False
    flags = str(row.get("Flags") or row.get("Flag") or "").strip()
    flagged = bool(flags and flags != "—")
    return (
        bool(re.search(r"review|weak|low confidence|late|early", f"{status} {flags}", re.IGNORECASE))
        or flagged
...
def compact_attendance_session(key: str, entry: dict, user: dict) -> dict | None:
    ...
    return {
        ...
        "needs_review_count": review_state["unresolved_count"],
        "absent_count": max(0, summary["total_students"] - summary["present_count"] - review_state["unresolved_count"]),
...
def home():
    return jsonify({
        ...
        "api": ["/api/health", "/api/profile", "/api/timetable", "/api/process", "/api/status", "/api/job/<id>", "/api/cancel/<id>", "/api/reset-processing", "/api/reports", "/api/videos", "/api/hod/overview", "/api/students"],
    })
...
def api_edit_attendance_session(session_id):
    ...
    with LOCK:
        ...
        summary["needs_review_count"] = review_state["unresolved_count"]
        summary["absent_count"] = max(0, summary["total_students"] - summary["present_count"] - review_state["unresolved_count"])
