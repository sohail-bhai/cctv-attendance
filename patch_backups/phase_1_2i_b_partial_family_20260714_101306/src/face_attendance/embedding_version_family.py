...
POLICY_VERSION = "phase-1.2i-b-v1.1"
...
def build_and_evaluate_embedding_family(
    *,
    ...
    try:
        ...
        if not staging_manifest["minimum_usable_evidence_passed"]:
            raise EmbeddingFamilyError(
                "Approved CCTV usability audit completed, but at least one required "
                "cross-session adaptation source has no technically usable crop: "
                + ", ".join(staging_manifest["missing_usable_sessions"])
            )
...
def verify_embedding_family(
    ...
    *,
    ...
    for key, definition in definitions.items():
        ...
        if included != set(definition.included_cctv_sessions):
            raise EmbeddingFamilyError(
                f"Variant {key} included-session contract changed: {sorted(included)}"
            )
        if allowed != set(definition.evaluation_sessions):
            raise EmbeddingFamilyError(
                f"Variant {key} evaluation-session contract changed: {sorted(allowed)}"
            )
        if contaminated != set(definition.contaminated_sessions):
            raise EmbeddingFamilyError(
