...
SCHEMA_VERSION = 1
POLICY_VERSION = "phase-1.2j-mon-p3-shadow-v1"
...
def _read_diagnostic_artifacts(
    ...
    if len(input_files) != 10:
        raise MonP3ShadowError(f"Diagnostic summary must contain ten videos; found {len(input_files)}")
    expected_names = sorted(Path(row["relative_path"]).name.lower() for row in preflight.freeze.rows)
    actual_names = sorted(_text(row.get("source_file_name")).lower() for row in input_files)
    if expected_names != actual_names:
        raise MonP3ShadowError("Diagnostic source-file names do not match the frozen MON_P3 layout")
