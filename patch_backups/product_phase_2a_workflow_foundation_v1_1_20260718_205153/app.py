...
def export_reviewed_attendance(entry: dict, rows: list[dict]) -> Path:
    ...
    path = run_dir / f"admin_reviewed_{safe_session}_{timestamp}.csv"
...
def apply_overrides(key: str, rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        item = dict(row)
        roll = str(item.get("Roll_Number", "")).upper()
...
def public_job(job: dict) -> dict:
    ...
    if not job:
        return {}
    hidden = {"process"}
    # Internal counters are useful for computing live progress, but they should
    # not be exposed to the frontend/API response.
    return {k: v for k, v in job.items() if k not in hidden and not str(k).startswith("_")}
...
def run_slot_job(job_id: str, day: str, period: str | int, payload: dict | None = None, reprocess: bool = False) -> None:
    ...
    day = normalize_day(day)
    try:
        row, video_dir, session = resolve_processing_context(day, period, payload, None)
    except Exception as exc:
    ...
    command = [
        ...
        "--subject-abbr", session["subject_abbr"],
        "--subject-name", session.get("subject_name", ""),
        "--course-code", session.get("course_code", ""),
        "--faculty-id", session.get("faculty_id", ""),
        "--faculty-name", session.get("faculty_name", ""),
        "--input-slot", session.get("input_slot", slot_id),
        "--input-source-type", session.get("input_source_type", ""),
        "--input-source-path", session.get("input_source_path", ""),
    ]
    ...
    try:
        ...
        entry_status = "Needs Review" if quality_status.get("requires_manual_review") else "Completed"
        ...
        entry = {
            **session,
            ...
            "output_manifest": {
                ...
                "attendance_xlsx": output_ref(excel_file),
            },
            "attendance_csv": output_ref(attendance_csv),
            "slot_summary_csv": output_ref(summary_csv),
            "detection_log_csv": output_ref(detection_csv),
        ...
        set_job(
            ...
            progress_text=quality_status.get("run_quality_status") or "Completed",
...
def start_job(day: str, period: str | int, payload: dict | None = None, reprocess: bool = False, user: dict | None = None) -> str:
    row, video_dir, session = resolve_processing_context(day, period, payload or {}, user)
...
def api_process():
    ...
    try:
        ...
        job_id = start_job(day, period, payload, reprocess=False, user=user)
...
def api_reprocess():
    ...
    try:
        job_id = start_job(day, period, payload, reprocess=True, user=user)
...
def api_cancel_job(job_id):
    ...
    set_job(
        ...
        progress_step=0,
        progress_percent=0,
        progress_text="Cancelled by Main Admin",
    )
    statuses = load_statuses()
    key = job.get("key")
...
def api_get_attendance_session(session_id):
    ...
    return jsonify({"success": True, "entry": entry, "attendance_data": rows, "overrides": load_overrides().get(session_id, {}), "admin": ADMIN_PROFILE, "user": public_user(user)})
...
def api_edit_attendance_session(session_id):
    ...
    session_overrides = overrides.get(session_id, {})
    ...
    if session_overrides:
        overrides[session_id] = session_overrides
    else:
        overrides.pop(session_id, None)
    save_overrides(overrides)
    edited_rows_all = apply_overrides(session_id, entry.get("attendance_data", []))
    ...
    statuses = load_statuses()
    updated = dict(statuses.get(session_id, entry))
    updated.update({
        "report_type": "Edited",
    ...
    statuses[session_id] = updated
    save_statuses(statuses)
    return jsonify({"success": True, "message": "Corrections saved and reviewed CSV exported.", "entry": updated, "admin_reviewed_csv": output_ref(export_path)})
