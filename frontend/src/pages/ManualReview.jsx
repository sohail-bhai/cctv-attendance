...
export default function ManualReview() {
  ...
  useEffect(() => {
    ...
    async function loadPeriods() {
      ...
      const opts = visible.map((row, index) => {
        ...
        return {
          value: normalizePeriod(row.period),
          key: `${normalizePeriod(row.period)}-${subject}-${index}`,
      ...
      if (opts.length && !opts.some((opt) => opt.value === period)) setPeriod(opts[0].value);
    }
    loadPeriods();
    return () => { cancelled = true; };
  }, [day, currentUser, period]);

  const load = async () => {
    setLoading(true);
    setMessage({ type: 'info', text: `Loading ${day} ${period} attendance...` });
    const data = await apiGet(`/api/attendance/${day}/${period}`, null);
    if (!data?.success) {
      setEntry(null);
      setRows([]);
      setChanges({});
  ...
  const save = async () => {
    ...
    try {
      const data = await apiPost(`/api/attendance/${day}/${period}/edit`, { changes: payload });
  ...
  const exportReviewed = async () => {
    const data = await apiGet(`/api/attendance/${day}/${period}/export-edited`, null);
