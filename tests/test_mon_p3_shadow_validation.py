...
import src.face_attendance.mon_p3_shadow_validation as mon_p3_module
...
class ManifestAndResumeRegressionTests(unittest.TestCase):
    def test_output_manifest_calls_never_pass_reserved_metadata_keys(self):
        tree = ast.parse(inspect.getsource(mon_p3_module))
    ...
    @staticmethod
    def _prepare_required_inputs(preflight: MonP3Preflight) -> None:
        for path, payload in (
    ...
    def test_complete_existing_diagnostics_resume_without_running_recognition(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with (
                ...
                mock.patch.object(
                    mon_p3_module, "compare_diagnostic_runs", return_value=comparison
                ),
                mock.patch.object(
                    mon_p3_module,
                ...
                mock.patch.object(mon_p3_module, "_run_command") as run_command,
            ):
                result = run_mon_p3_shadow(preflight)
    ...
    def test_only_one_existing_diagnostic_side_fails_without_recognition(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with mock.patch.object(mon_p3_module, "_run_command") as run_command:
                with self.assertRaisesRegex(MonP3ShadowError, "only one diagnostic side"):
                    run_mon_p3_shadow(preflight)
            run_command.assert_not_called()
