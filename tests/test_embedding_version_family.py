...
from src.face_attendance.embedding_version_family import (
    ...
    _effective_variant_definition,
    _write_variant,
    _write_unavailable_variant,
...
class VariantAvailabilityTests(unittest.TestCase):
    def test_missing_tue_p2_evidence_marks_only_dependent_variant_unavailable(self) -> None:
        plan = resolve_variant_availability(
            {
                "embedding_usable_session_counts": {
                    TUE_P1_SESSION: 6,
                    TUE_P2_SESSION: 0,
                }
            }
        )
        self.assertTrue(plan["A"]["available"])
        self.assertFalse(plan["B"]["available"])
        self.assertTrue(plan["C"]["available"])
        self.assertTrue(plan["D"]["available"])
        self.assertEqual(plan["D"]["source_sessions_included"], [TUE_P1_SESSION])
        self.assertEqual(
            plan["B"]["missing_usable_source_sessions"], [TUE_P2_SESSION]
        )
        self.assertFalse(plan["summary"]["cross_session_pair_complete"])
        self.assertFalse(plan["summary"]["quality_gate_relaxed"])
        self.assertFalse(plan["summary"]["manual_rereview_required"])

    def test_no_usable_cctv_evidence_still_preserves_enrollment_only_variant(self) -> None:
        plan = resolve_variant_availability(
            {"embedding_usable_session_counts": {}}
        )
        self.assertTrue(plan["A"]["available"])
        self.assertFalse(plan["B"]["available"])
        self.assertFalse(plan["C"]["available"])
        self.assertFalse(plan["D"]["available"])
        self.assertEqual(plan["summary"]["available_variants"], ["A"])

    def test_unavailable_variant_writes_no_embedding_payload(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            plan = resolve_variant_availability(
                {
            ...
            from src.face_attendance.embedding_version_family import VARIANT_DEFINITIONS

            definition = next(value for value in VARIANT_DEFINITIONS if value.key == "B")
            manifest = _write_unavailable_variant(
                temporary_variant_dir=root / "variant-b",
                family_id="embfam-test",
                definition=definition,
                availability=plan["B"],
                parent_embeddings_sha256="a" * 64,
                parent_summary_sha256="b" * 64,
                model_hashes={"yunet_sha256": "c" * 64, "sface_sha256": "d" * 64},
                source_manifest_sha256="e" * 64,
                enrollment_approval_sha256="f" * 64,
                cctv_approval_sha256="1" * 64,
                identity_correction_sha256="2" * 64,
                augmentation_policy_sha256="3" * 64,
                roster_rolls={"2401100CSE0001"},
                build_timestamp="2026-07-14T00:00:00+05:30",
                expected_dimension=128,
            )
            self.assertFalse(manifest["availability"])
            self.assertEqual(
                manifest["status"], "unavailable_no_usable_source_evidence"
            )
            self.assertEqual(manifest["embedding_records"], 0)
            self.assertFalse((root / "variant-b" / "student_embeddings.pkl").exists())
            self.assertTrue((root / "variant-b" / "unavailable_report.json").is_file())

    def test_partial_full_candidate_manifest_records_only_actual_usable_session(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            plan = resolve_variant_availability(
                {
            ...
            base = next(value for value in VARIANT_DEFINITIONS if value.key == "D")
            definition = _effective_variant_definition(base, plan["D"])
            vector = np.asarray([1.0, 0.0, 0.0], dtype=np.float32)
            manifest = _write_variant(
                temporary_variant_dir=root / "variant-d",
                final_variant_dir=root / "final" / "variant-d",
                family_id="embfam-test",
                definition=definition,
                family_fingerprint="4" * 64,
                records=[
                    {
                        "roll_no": "2401100CSE0001",
            ...
            self.assertTrue(manifest["availability"])
            self.assertEqual(manifest["source_sessions_included"], [TUE_P1_SESSION])
            self.assertEqual(
                manifest["missing_usable_source_sessions"], [TUE_P2_SESSION]
            )
            self.assertEqual(manifest["adaptation_completeness"], "partial")
            self.assertEqual(
                manifest["evaluation_sessions_contaminated"], [TUE_P1_SESSION]
            )
...
class SafetyGateTests(unittest.TestCase):
    ...
    def test_missing_tue_p2_adaptation_is_insufficient_not_integrity_failure(self) -> None:
        baseline, variants = self._safe_frames()
        variants["B"] = baseline.head(0).copy()
        availability = resolve_variant_availability(
            {
                "embedding_usable_session_counts": {
                    TUE_P1_SESSION: 6,
                    TUE_P2_SESSION: 0,
        ...
        manifests["B"].update(
            {
                "availability": False,
                "status": "unavailable_no_usable_source_evidence",
                "source_sessions_included": [],
                "evaluation_sessions_allowed": [],
                "evaluation_sessions_contaminated": [],
                "unavailable_reason": availability["B"]["unavailable_reason"],
            }
        )
        manifests["D"]["source_sessions_included"] = [TUE_P1_SESSION]
        ...
        self.assertEqual(
            decision["decision"], "built_unapproved_insufficient_evidence"
        )
        self.assertTrue(decision["gates"]["evaluation_structure"]["passed"])
        self.assertFalse(decision["gates"]["evidence_sufficiency"]["passed"])
        self.assertFalse(
            decision["gates"]["reviewed_accepted_track_retention"][
                "decisive_retention_evaluable"
            ]
        )
        self.assertIn("B", decision["unavailable_variants"])
        self.assertEqual(set(comparisons["Comparison_ID"]), {
            "production_vs_variant_a",
            "production_vs_variant_c",
        })

    def test_unavailable_variant_cannot_hide_false_identity_in_available_variant(self) -> None:
        baseline, variants = self._safe_frames()
        variants["B"] = baseline.head(0).copy()
        p2_index = variants["C"].index[0]
        variants["C"].at[p2_index, "Accepted"] = True
        variants["C"].at[p2_index, "Accepted_Roll"] = "2401100CSE0001"
        variants["C"].at[p2_index, "Outsider_Absorption"] = True
        availability = resolve_variant_availability(
            {
                "embedding_usable_session_counts": {
                    TUE_P1_SESSION: 6,
        ...
        manifests = _variant_manifests()
        manifests["B"]["source_sessions_included"] = []
        manifests["B"]["evaluation_sessions_allowed"] = []
        manifests["D"]["source_sessions_included"] = [TUE_P1_SESSION]
        decision, _ = apply_family_safety_gates(
            baseline=baseline,
            variant_predictions=variants,
