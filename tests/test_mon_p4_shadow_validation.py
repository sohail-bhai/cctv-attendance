...
import src.face_attendance.mon_p4_shadow_validation as mon_p4_module
from src.face_attendance.mon_p4_shadow_validation import (
...
class SourceFreezeTests(unittest.TestCase):
    ...
    def test_canonical_video_mutation_fails_closed(self):
        with tempfile.TemporaryDirectory() as temp:
            canonical, mirror, json_path, csv_path = _freeze_fixture(Path(temp))
            (canonical / "CP1_1150" / "back.mp4").write_bytes(b"tampered")
            with self.assertRaisesRegex(MonP4ShadowError, "changed after source freeze"):
                verify_source_freeze(
                    freeze_json_path=json_path,
                    freeze_csv_path=csv_path,
                    canonical_root=canonical,
    ...
    def test_mirror_mismatch_fails_closed(self):
        with tempfile.TemporaryDirectory() as temp:
            canonical, mirror, json_path, csv_path = _freeze_fixture(Path(temp))
            (mirror / "CP2_1200" / "front.mp4").write_bytes(b"different")
            with self.assertRaises(MonP4ShadowError):
                verify_source_freeze(
                    freeze_json_path=json_path,
                    freeze_csv_path=csv_path,
                    canonical_root=canonical,
    ...
    def test_freeze_claiming_recognition_was_executed_is_rejected(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with self.assertRaisesRegex(MonP4ShadowError, "claims recognition"):
                verify_source_freeze(
                    freeze_json_path=json_path,
                    freeze_csv_path=csv_path,
                    canonical_root=canonical,
...
class SourceFreezeCreationTests(unittest.TestCase):
    def _expected_inventory(self, root: Path, canonical: Path) -> Path:
        rows = []
        for relative in EXPECTED_RELATIVE_VIDEOS:
            path = canonical / Path(relative.replace("\\", "/"))
            rows.append(
                {
                    "RelativePath": relative,
        ...
        inventory = root / "expected_inventory.csv"
        pd.DataFrame(rows).to_csv(inventory, index=False)
        return inventory

    def test_create_source_freeze_verifies_inventory_mirror_and_idempotency(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            repo = root / "repo"
            canonical, mirror, _, _ = _freeze_fixture(root / "fixture")
            inventory = self._expected_inventory(root, canonical)
            family = repo / "models" / "versions" / "family"
            family.mkdir(parents=True)
            output_root = repo / "attendance_output" / "shadow_validation" / "phase_1_2n" / "source_freeze"
            first = create_mon_p4_source_freeze(
                repo_root=repo,
                family_dir=family,
                canonical_root=canonical,
                mirror_root=mirror,
                expected_inventory_csv=inventory,
                output_root=output_root,
            )
            second = create_mon_p4_source_freeze(
                repo_root=repo,
                family_dir=family,
                canonical_root=canonical,
                mirror_root=mirror,
            ...
            self.assertTrue(payload["UntouchedContractPassed"])
            self.assertFalse(payload["RecognitionExecuted"])

    def test_prior_mon_p4_output_blocks_untouched_freeze(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            repo = root / "repo"
            canonical, mirror, _, _ = _freeze_fixture(root / "fixture")
            ...
            family = repo / "models" / "versions" / "family"
            family.mkdir(parents=True)
            prior = repo / "attendance_output" / f"{EXPECTED_SESSION_ID}_old_run"
            prior.mkdir(parents=True)
            with self.assertRaisesRegex(MonP4ShadowError, "cannot be certified untouched"):
                create_mon_p4_source_freeze(
                    repo_root=repo,
                    family_dir=family,
                    canonical_root=canonical,
    ...
    def test_expected_inventory_hash_mismatch_fails_closed(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            repo = root / "repo"
            canonical, mirror, _, _ = _freeze_fixture(root / "fixture")
            ...
            family = repo / "models" / "versions" / "family"
            family.mkdir(parents=True)
            with self.assertRaisesRegex(MonP4ShadowError, "differs from the uploaded evidence inventory"):
                create_mon_p4_source_freeze(
                    repo_root=repo,
                    family_dir=family,
                    canonical_root=canonical,
...
class CandidateFamilyTests(unittest.TestCase):
    def _family_fixture(self, root: Path) -> tuple[Path, Path, Path]:
        ...
        (family / "evaluation" / "evaluation_decision.json").write_text(
            json.dumps(
                {
                    "exact_next_step": "Reserve and run a different untouched validation session.",
                    "production_approved": False,
                    "candidate_promoted": False,
                    "mon_p4_processed": False,
                    "new_untouched_session_required": True,
    ...
    def test_full_candidate_uses_version_id_and_positive_record_count(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with mock.patch(
                "src.face_attendance.mon_p4_shadow_validation.verify_embedding_family",
                return_value={"decision": "built_unapproved_pending_new_untouched_session"},
            ):
    ...
    def test_full_candidate_hash_mismatch_fails_closed(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with mock.patch(
                ...
                with self.assertRaisesRegex(MonP4ShadowError, "hash does not match"):
                    verify_candidate_family(
                        family_dir=family,
                        production_embeddings=production,
                        production_summary=summary,
...
class ComparisonTests(unittest.TestCase):
    ...
    def test_tracklet_structure_change_fails_integrity(self):
        ...
        with self.assertRaisesRegex(MonP4ShadowError, "structural column Checkpoint_ID"):
...
class CommandContractTests(unittest.TestCase):
    def _preflight(self, root: Path) -> MonP4Preflight:
    ...
    def test_command_is_diagnostic_only_and_uses_mon_p4(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
...
class DiagnosticSourceLayoutTests(unittest.TestCase):
    def _preflight_with_frozen_videos(self, root: Path) -> MonP4Preflight:
    ...
    def _valid_summary(self, preflight: MonP4Preflight) -> dict:
        input_files = []
        timing_rows = []
    ...
    def test_cp1_repeated_and_cp2_missing_fails_closed(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            cp1_rows = [
                ...
                if "CP1_1150" in row["source_file"]
            ]
            summary["input_files"] = cp1_rows + cp1_rows + [
                row
                for row in summary["input_files"]
                if "CP1_1150" not in row["source_file"]
                and "CP2_1200" not in row["source_file"]
            ]
            with self.assertRaisesRegex(MonP4ShadowError, "frozen ten-video checkpoint set"):
    ...
    def test_checkpoint_id_must_match_source_folder(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with self.assertRaisesRegex(MonP4ShadowError, "folder mapping is invalid"):
                _validate_diagnostic_source_layout(summary, preflight)



class ManifestAndResumeRegressionTests(unittest.TestCase):
    def test_output_manifest_calls_never_pass_reserved_metadata_keys(self):
        tree = ast.parse(inspect.getsource(mon_p4_module))
        reserved = {"schema_version", "created_at", "output_dir", "files_sha256"}
        calls = 0
        for node in ast.walk(tree):
            if not isinstance(node, ast.Call):
                continue
            if not isinstance(node.func, ast.Name) or node.func.id != "write_output_manifest":
                continue
            calls += 1
            self.assertGreaterEqual(len(node.args), 2)
            metadata = node.args[1]
            self.assertIsInstance(metadata, ast.Dict)
            keys = {
                key.value
            ...
            self.assertFalse(
                reserved.intersection(keys),
                f"Reserved output-manifest metadata keys were passed: {sorted(reserved.intersection(keys))}",
            )
        self.assertGreaterEqual(calls, 3)

    @staticmethod
    def _diagnostic_artifacts(root: Path, label: str) -> DiagnosticArtifacts:
        run = root / label
        run.mkdir(parents=True)
        summary = run / f"diagnostic_summary_{label}.json"
        faces = run / f"face_diagnostics_{label}.csv"
        tracklets = run / f"tracklet_diagnostics_{label}.csv"
        observations = run / f"tracklet_observations_{label}.csv"
        summary.write_text("{}", encoding="utf-8")
        pd.DataFrame(columns=["Face_ID"]).to_csv(faces, index=False)
        pd.DataFrame(columns=["Tracklet_ID"]).to_csv(tracklets, index=False)
        pd.DataFrame(columns=["Observation_ID", "Tracklet_ID"]).to_csv(
        ...
        return DiagnosticArtifacts(
            ...
            face_path=faces,
            tracklet_path=tracklets,
            observations_path=observations,
            summary={},
            faces=pd.DataFrame(columns=["Face_ID"]),
            tracklets=pd.DataFrame(columns=["Tracklet_ID"]),
            observations=pd.DataFrame(columns=["Observation_ID", "Tracklet_ID"]),
        )

    @staticmethod
    def _prepare_required_inputs(preflight: MonP4Preflight) -> None:
        for path, payload in (
            (preflight.freeze.json_path, "{}"),
            (preflight.freeze.csv_path, "RelativePath\n"),
            (preflight.production_embeddings, "production"),
            (preflight.production_summary, "roll,count\n"),
            (preflight.student_map, "{}"),
            (preflight.timetable, "slot\n"),
            (preflight.family.candidate_embeddings, "candidate"),
            (preflight.family.family_dir / "output_manifest.json", "{}"),
            (preflight.family.family_dir / "family_manifest.json", "{}"),
            (preflight.family.family_dir / "evaluation" / "evaluation_decision.json", "{}"),
            (
                preflight.family.family_dir
                / "variants"
                / "full_candidate"
    ...
    def test_complete_existing_diagnostics_resume_without_running_recognition(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            preflight = CommandContractTests()._preflight(root)
            self._prepare_required_inputs(preflight)
            production = self._diagnostic_artifacts(
                preflight.production_diagnostic_run.parent,
                preflight.production_diagnostic_run.name,
            )
            candidate = self._diagnostic_artifacts(
                preflight.candidate_diagnostic_run.parent,
                preflight.candidate_diagnostic_run.name,
            )
            comparison = ComparisonResult(
                tracklets=pd.DataFrame(columns=["Tracklet_ID"]),
                observations=pd.DataFrame(columns=["Observation_ID", "Tracklet_ID"]),
                faces=pd.DataFrame(columns=["Face_ID"]),
                exception_tracks=pd.DataFrame(columns=["Tracklet_ID"]),
                frame_exception_rows=pd.DataFrame(columns=["Tracklet_ID"]),
                student_checkpoint=pd.DataFrame(columns=["Student_ID"]),
                session_summary={
                    "unmapped_official_frame_deltas": 0,
                    "evidence_sufficient": True,
                },
            )

            def fake_export_review_package(**kwargs):
                review_root = Path(kwargs["output_root"]) / "review-package"
                review_root.mkdir(parents=True)
                return types.SimpleNamespace(root=review_root, package_id="review-test")

            with (
                mock.patch.object(
                    mon_p4_module,
                    "_read_diagnostic_artifacts",
                    side_effect=[production, candidate],
                ),
                mock.patch.object(
                    mon_p4_module, "compare_diagnostic_runs", return_value=comparison
                ),
                mock.patch.object(
                    mon_p4_module,
                    "snapshot_protected_state",
                    return_value={"protected": "unchanged"},
                ),
                mock.patch.object(
                    mon_p4_module,
                    "export_review_package",
                    side_effect=fake_export_review_package,
                ),
                mock.patch.object(mon_p4_module, "_run_command") as run_command,
            ):
                result = run_mon_p4_shadow(preflight)

            run_command.assert_not_called()
            self.assertTrue(result.output_dir.is_dir())
            self.assertTrue(result.summary["recognition_reused_from_existing_diagnostics"])
            self.assertFalse(result.summary["recognition_executed_in_this_invocation"])
            self.assertEqual(
                result.summary["postprocessing_revision"], POSTPROCESSING_REVISION
            )
            manifest = json.loads(
                (result.output_dir / "output_manifest.json").read_text(encoding="utf-8")
            )
            self.assertEqual(manifest["schema_version"], 1)
            self.assertEqual(manifest["postprocessing_revision"], POSTPROCESSING_REVISION)
            self.assertTrue(manifest["recognition_reused_from_existing_diagnostics"])

    def test_only_one_existing_diagnostic_side_fails_without_recognition(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            preflight = CommandContractTests()._preflight(root)
            preflight.production_diagnostic_run.mkdir(parents=True)
            with mock.patch.object(mon_p4_module, "_run_command") as run_command:
                with self.assertRaisesRegex(MonP4ShadowError, "only one diagnostic side"):
                    run_mon_p4_shadow(preflight)
            run_command.assert_not_called()
