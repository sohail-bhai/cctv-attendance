...
POSTPROCESSING_REVISION = "phase-1.2j-manifest-resume-v1"
...
def run_mon_p3_shadow(
    ...
    *,
    ...
    if any(diagnostic_presence) and not all(diagnostic_presence):
        ...
        raise MonP3ShadowError(
            "Phase 1.2J has only one diagnostic side; refusing recognition and resume. "
    ...
    if resume_from_existing_diagnostics:
        if status_callback:
            status_callback(
                "Reusing both existing, strictly validated diagnostic-only MON_P3 runs after "
                "a post-processing failure. Recognition will not run again."
            )
    else:
        if status_callback:
            status_callback(
                "Running production diagnostic-only MON_P3 pass. Official attendance remains disabled."
            )
        _run_command(production_command, cwd=preflight.repo_root, status_callback=status_callback)
        if status_callback:
            status_callback(
                "Running candidate diagnostic-only MON_P3 pass. Candidate remains unpromoted."
            )
    ...
    if diagnostic_hashes_after != diagnostic_hashes_before:
        raise MonP3ShadowError(
            "Diagnostic evidence changed during Phase 1.2J post-processing; refusing finalization"
        )
