...
SCHEMA_VERSION = 1
POLICY_VERSION = "phase-1.2k-retention-attribution-v1.1"
...
def _required_int(payload: dict[str, Any], key: str) -> int:
    if key not in payload:
        raise RetentionAttributionError(f"Required integer field missing: {key}")
    value = payload[key]
    if isinstance(value, bool):
        raise RetentionAttributionError(f"Required integer field is boolean: {key}")
    try:
        numeric = float(value)
    except (TypeError, ValueError) as exc:
        raise RetentionAttributionError(f"Required integer field is invalid: {key}") from exc
    if not math.isfinite(numeric) or not numeric.is_integer():
        raise RetentionAttributionError(f"Required integer field is invalid: {key}")
    return int(numeric)
...
def resolve_inputs(
    *,
    ...
    if _required_int(evaluation, "candidate_lost_correct_production_accepts") != EXPECTED_RETENTION_LOSSES:
        raise RetentionAttributionError("Frozen evaluation no longer contains exactly six retention losses")
    if _required_int(evaluation, "candidate_correct_recoveries") != EXPECTED_CORRECT_RECOVERIES:
        raise RetentionAttributionError("Frozen evaluation no longer contains exactly four correct recoveries")
    if _required_int(evaluation, "candidate_false_identities_or_unsafe_accepts") != EXPECTED_FALSE_IDENTITIES:
