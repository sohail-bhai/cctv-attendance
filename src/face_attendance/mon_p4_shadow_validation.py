...
SCHEMA_VERSION = 1
POLICY_VERSION = "phase-1.2n-mon-p4-shadow-v1"
POSTPROCESSING_REVISION = "phase-1.2n-mon-p4-shadow-v1"
EXPECTED_SESSION_ID = "2026-06-22__B51__P4__CVO"
EXPECTED_SLOT_ID = "MON_P4"
...
class MonP4ShadowError(ValueError):
    pass
...
def _checkpoint_id_from_relative_path(relative_path: str) -> str:
    ...
    if match is None:
        raise MonP4ShadowError(
            f"Frozen MON_P4 relative path does not start with a checkpoint folder: {relative_path}"
        )
...
def _load_json(path: Path, label: str) -> dict[str, Any]:
    path = Path(path)
    if not path.is_file():
        raise MonP4ShadowError(f"{label} not found: {path}")
    try:
        payload = json.loads(path.read_text(encoding="utf-8-sig"))
    except (OSError, json.JSONDecodeError) as exc:
        raise MonP4ShadowError(f"Could not read {label}: {exc}") from exc
    if not isinstance(payload, dict):
        raise MonP4ShadowError(f"{label} must contain a JSON object")
    return payload
...
def _read_csv_or_empty(path: Path, columns: Iterable[str]) -> pd.DataFrame:
    try:
        ...
        raise MonP4ShadowError(f"Could not read CSV {path}: {exc}") from exc
...
def _parse_bool(value: Any) -> bool:
    ...
    if text in {"false", "no", "0"}:
        return False
    raise MonP4ShadowError(f"Invalid Boolean value in source freeze: {value!r}")



EXPECTED_INVENTORY_COLUMNS = {"RelativePath", "SizeBytes", "SHA256"}


def _read_expected_inventory(path: Path) -> pd.DataFrame:
    path = Path(path).resolve()
    if not path.is_file():
        raise MonP4ShadowError(f"Expected MON_P4 inventory not found: {path}")
    try:
        frame = pd.read_csv(path, dtype=str, keep_default_na=False, encoding="utf-8-sig")
    except Exception as exc:
        raise MonP4ShadowError(f"Could not read expected MON_P4 inventory: {exc}") from exc
    missing = sorted(EXPECTED_INVENTORY_COLUMNS.difference(frame.columns))
    if missing:
        raise MonP4ShadowError(
            "Expected MON_P4 inventory is missing columns: " + ", ".join(missing)
        )
    if len(frame) != 10:
        raise MonP4ShadowError(f"Expected MON_P4 inventory must contain ten rows; found {len(frame)}")
    if frame["RelativePath"].duplicated().any():
        raise MonP4ShadowError("Expected MON_P4 inventory contains duplicate relative paths")
    actual = sorted(frame["RelativePath"].astype(str).str.replace("/", "\\", regex=False))
    expected = sorted(EXPECTED_RELATIVE_VIDEOS)
    if actual != expected:
        raise MonP4ShadowError(
            f"Expected MON_P4 inventory path set changed; expected={expected}, actual={actual}"
        )
    hashes = frame["SHA256"].astype(str).str.lower()
    if hashes.eq("").any() or hashes.duplicated().any():
        raise MonP4ShadowError("Expected MON_P4 inventory hashes must be non-empty and unique")
    for value in frame["SizeBytes"].astype(str):
        if _int(value, -1) <= 0:
            raise MonP4ShadowError("Expected MON_P4 inventory contains an invalid file size")
    return frame


def _scan_untouched_contract(*, repo_root: Path, family_dir: Path, output_root: Path) -> dict[str, Any]:
    ...
    path_hits: list[str] = []
    markers = (
        EXPECTED_SESSION_ID.lower(),
        EXPECTED_SLOT_ID.lower(),
        "mon-p4",
        "mon_p4",
    )
    if attendance_root.exists():
        for item in attendance_root.rglob("*"):
            try:
                rel = item.relative_to(attendance_root).as_posix()
            except ValueError:
                continue
            lower = rel.lower()
            # Phase 1.2N owns this output root and must be safely reusable.
            if lower.startswith("shadow_validation/phase_1_2n/"):
                continue
    ...
    if versions_root.exists():
        candidate_files = list(versions_root.rglob("source_records.csv"))
        candidate_files.extend(versions_root.rglob("candidate_source_selection.json"))
        candidate_files.extend(versions_root.rglob("recovery_consumption_sources.csv"))
        for path in sorted(set(candidate_files)):
            try:
                text = path.read_text(encoding="utf-8-sig", errors="replace").lower()
            except OSError as exc:
                raise MonP4ShadowError(f"Could not inspect candidate source provenance: {path}: {exc}") from exc
            if EXPECTED_SESSION_ID.lower() in text or EXPECTED_SLOT_ID.lower() in text:
                source_hits.append(str(path.relative_to(repo_root)))
                if len(source_hits) >= 50:
                    break

    if path_hits or source_hits:
        details = []
        if path_hits:
            details.append("attendance-output paths=" + ", ".join(path_hits[:10]))
        if source_hits:
            details.append("candidate-source files=" + ", ".join(source_hits[:10]))
        raise MonP4ShadowError(
            "MON_P4 cannot be certified untouched because prior derived evidence was found: "
            + "; ".join(details)
        )
    return {
        "attendance_output_scanned": str(attendance_root),
        "candidate_versions_scanned": str(versions_root),
        "prior_mon_p4_path_hits": [],
        "prior_mon_p4_source_hits": [],
        "untouched_contract_passed": True,
    }


def create_mon_p4_source_freeze(
    *,
    repo_root: Path,
    family_dir: Path,
    canonical_root: Path,
    mirror_root: Path,
    expected_inventory_csv: Path,
    output_root: Path,
    ...
    expected_inventory_csv = Path(expected_inventory_csv).resolve()
    output_root = Path(output_root).resolve()
    if not canonical_root.is_dir() or not mirror_root.is_dir():
        raise MonP4ShadowError(
            f"MON_P4 canonical and mirror roots are required: canonical={canonical_root}, mirror={mirror_root}"
        )
    expected = _read_expected_inventory(expected_inventory_csv)
    actual_canonical = sorted(
        path.relative_to(canonical_root).as_posix().replace("/", "\\")
        for path in canonical_root.rglob("*.mp4")
        if path.is_file()
    )
    actual_mirror = sorted(
        path.relative_to(mirror_root).as_posix().replace("/", "\\")
        for path in mirror_root.rglob("*.mp4")
        if path.is_file()
    )
    expected_paths = sorted(EXPECTED_RELATIVE_VIDEOS)
    if actual_canonical != expected_paths:
        raise MonP4ShadowError(
            f"Canonical MON_P4 layout is not exact; expected={expected_paths}, actual={actual_canonical}"
        )
    if actual_mirror != expected_paths:
        raise MonP4ShadowError(
            f"Mirror MON_P4 layout is not exact; expected={expected_paths}, actual={actual_mirror}"
        )

    expected_by_path = {
        _text(row["RelativePath"]).replace("/", "\\"): row
    ...
    for relative in EXPECTED_RELATIVE_VIDEOS:
        ...
        expected_row = expected_by_path[relative]
        ...
        expected_length = _int(expected_row["SizeBytes"], -1)
        expected_hash = _text(expected_row["SHA256"]).lower()
        if canonical_length != expected_length or canonical_hash != expected_hash:
            raise MonP4ShadowError(
                f"Canonical MON_P4 source differs from the uploaded evidence inventory: {relative}"
            )
        if mirror_length != canonical_length or mirror_hash != canonical_hash:
            raise MonP4ShadowError(f"MON_P4 mirror is not byte-identical: {relative}")
        rows.append(
            {
                "RelativePath": relative,
                "CanonicalPath": str(canonical_path),
                "CanonicalLength": canonical_length,
                "CanonicalSHA256": canonical_hash,
                "MirrorPath": str(mirror_path),
                "MirrorLength": mirror_length,
                "MirrorSHA256": mirror_hash,
                "ByteIdentical": True,
            }
        )
    if len({row["CanonicalSHA256"] for row in rows}) != 10:
        raise MonP4ShadowError("MON_P4 source contains duplicate video content")
    ...
    fingerprint = _stable_digest(
        {
            ...
            "expected_inventory_sha256": _sha256_file(expected_inventory_csv),
            "videos": {row["RelativePath"]: row["CanonicalSHA256"] for row in rows},
            "untouched_contract_required": True,
        }
    )
    freeze_id = f"source-freeze-{fingerprint[:20]}"
    final_dir = output_root / freeze_id
    freeze_json = final_dir / "phase_1_2n_mon_p4_source_freeze.json"
    freeze_csv = final_dir / "phase_1_2n_mon_p4_source_freeze.csv"
    if final_dir.exists():
        try:
            verify_output_manifest(final_dir, final_dir / "output_manifest.json")
        except ShadowValidationError as exc:
            raise MonP4ShadowError(str(exc)) from exc
        ...
        return final_dir, freeze_json, freeze_csv

    untouched = _scan_untouched_contract(
        repo_root=repo_root,
        family_dir=family_dir,
        output_root=output_root,
    )
    output_root.mkdir(parents=True, exist_ok=True)
    temporary = output_root / f".{freeze_id}.tmp-{os.getpid()}-{time.time_ns()}"
    temporary.mkdir(parents=False, exist_ok=False)
    final_json = final_dir / freeze_json.name
    final_csv = final_dir / freeze_csv.name
    pd.DataFrame(rows).to_csv(temporary / freeze_csv.name, index=False, encoding="utf-8")
    payload = {
        "SchemaVersion": 1,
        "Phase": "1.2N",
        ...
        "SubjectAbbr": EXPECTED_SUBJECT,
        "CanonicalVideoRoot": str(canonical_root),
        "MirrorVideoRoot": str(mirror_root),
        ...
        "UntouchedScan": untouched,
        "ExpectedInventoryPath": str(expected_inventory_csv),
        "ExpectedInventorySHA256": _sha256_file(expected_inventory_csv),
    ...
    (temporary / freeze_json.name).write_text(
    ...
    manifest_path = final_dir / "output_manifest.json"
    manifest_payload = _load_json(manifest_path, "Phase 1.2N source-freeze output manifest")
    manifest_payload["output_dir"] = str(final_dir.resolve())
    manifest_path.write_text(json.dumps(json_safe(manifest_payload), indent=2), encoding="utf-8")
    verify_source_freeze(
        freeze_json_path=final_json,
        freeze_csv_path=final_csv,
        canonical_root=canonical_root,
        mirror_root=mirror_root,
    )
    return final_dir, final_json, final_csv
...
def verify_source_freeze(
    *,
    ...
    canonical_root = Path(canonical_root).resolve()
    mirror_root = Path(mirror_root).resolve()
    payload = _load_json(Path(freeze_json_path), "MON_P4 source-freeze JSON")
    csv_path = Path(freeze_csv_path)
    if not csv_path.is_file():
        raise MonP4ShadowError(f"MON_P4 source-freeze CSV not found: {csv_path}")
    try:
        rows_df = pd.read_csv(csv_path, dtype=str, keep_default_na=False, encoding="utf-8-sig")
    except Exception as exc:
        raise MonP4ShadowError(f"Could not read MON_P4 source-freeze CSV: {exc}") from exc
    missing_columns = sorted(REQUIRED_FREEZE_COLUMNS.difference(rows_df.columns))
    if missing_columns:
        raise MonP4ShadowError(
            "MON_P4 source-freeze CSV is missing columns: " + ", ".join(missing_columns)
        )
    if int(payload.get("SchemaVersion", -1)) != 1 or _text(payload.get("Phase")) != "1.2N":
        raise MonP4ShadowError("MON_P4 source-freeze JSON schema or phase is invalid")
    if _text(payload.get("SessionID")) != session_id:
        raise MonP4ShadowError("MON_P4 source-freeze session ID does not match the requested session")
    ...
    if any(payload.get(key) is not False for key in required_false):
        raise MonP4ShadowError("MON_P4 source freeze claims recognition, attendance, or promotion occurred")
    required_true = (
        "ExpectedLayoutPassed",
        "TreesByteIdentical",
        "SourceFreezePassed",
        "UntouchedContractPassed",
        "CanonicalSourceOnly",
    )
    if any(payload.get(key) is not True for key in required_true):
        raise MonP4ShadowError("MON_P4 source freeze did not pass all required safeguards")
    if int(payload.get("CanonicalVideoCount", -1)) != 10 or int(payload.get("MirrorVideoCount", -1)) != 10:
        raise MonP4ShadowError("MON_P4 source freeze must contain exactly ten canonical and ten mirror videos")
    if not _same_path(payload.get("CanonicalVideoRoot", ""), canonical_root):
        raise MonP4ShadowError("MON_P4 source-freeze canonical root does not match the requested root")
    if not _same_path(payload.get("MirrorVideoRoot", ""), mirror_root):
        raise MonP4ShadowError("MON_P4 source-freeze mirror root does not match the requested root")
    recorded_csv = _text(payload.get("CSV"))
    if not recorded_csv or not _same_path(recorded_csv, csv_path):
        raise MonP4ShadowError("MON_P4 source-freeze JSON does not point to the supplied CSV")
    if len(rows_df) != 10:
        raise MonP4ShadowError(f"MON_P4 source-freeze CSV must contain ten rows; found {len(rows_df)}")
    actual_relative = sorted(rows_df["RelativePath"].astype(str).tolist())
    expected_relative = sorted(EXPECTED_RELATIVE_VIDEOS)
    if actual_relative != expected_relative:
        raise MonP4ShadowError(
            f"MON_P4 source-freeze path set is invalid; expected={expected_relative}, actual={actual_relative}"
        )
    if rows_df["RelativePath"].duplicated().any():
        raise MonP4ShadowError("MON_P4 source-freeze CSV contains duplicate relative paths")
    ...
    for row in rows_df.to_dict("records"):
        ...
        if not canonical_path.is_file() or not mirror_path.is_file():
            raise MonP4ShadowError(f"Frozen MON_P4 source is missing: {relative}")
        if not _same_path(row["CanonicalPath"], canonical_path):
            raise MonP4ShadowError(f"Canonical path changed since source freeze: {relative}")
        if not _same_path(row["MirrorPath"], mirror_path):
            raise MonP4ShadowError(f"Mirror path changed since source freeze: {relative}")
        canonical_length = canonical_path.stat().st_size
        mirror_length = mirror_path.stat().st_size
        canonical_hash = _sha256_file(canonical_path)
        mirror_hash = _sha256_file(mirror_path)
        ...
        if canonical_length != recorded_canonical_length or mirror_length != recorded_mirror_length:
            raise MonP4ShadowError(f"MON_P4 video length changed after source freeze: {relative}")
        if canonical_hash != recorded_canonical_hash or mirror_hash != recorded_mirror_hash:
            raise MonP4ShadowError(f"MON_P4 video hash changed after source freeze: {relative}")
        if not _parse_bool(row["ByteIdentical"]):
            raise MonP4ShadowError(f"MON_P4 source freeze marks a non-identical mirror: {relative}")
        if canonical_length != mirror_length or canonical_hash != mirror_hash:
            raise MonP4ShadowError(f"MON_P4 canonical and mirror files are not byte-identical: {relative}")
    ...
    if len({row["sha256"] for row in verified_rows}) != len(verified_rows):
        raise MonP4ShadowError("MON_P4 source freeze contains duplicate video content")
...
def verify_candidate_family(
    *,
    ...
    if not family_output_manifest.is_file():
        raise MonP4ShadowError("Candidate family output manifest is missing")
    ...
    if evaluation_decision.get("new_untouched_session_required") is not True:
        raise MonP4ShadowError("Candidate family does not require a new untouched validation session")
    if family_manifest.get("new_untouched_session_required") is not True:
        raise MonP4ShadowError("Family manifest does not preserve the untouched-session requirement")
    if evaluation_decision.get("mon_p3_reuse_for_promotion_allowed") is not False:
        raise MonP4ShadowError("Family unexpectedly permits MON_P3 reuse as promotion evidence")
    if any(
        value is not False
        for value in (
        ...
        raise MonP4ShadowError("Candidate family violates the unapproved contract")
    ...
    if not candidate_embeddings.is_file():
        raise MonP4ShadowError("Full-candidate embedding payload is missing")
    if _text(variant_manifest.get("status")) != "built_unapproved":
        raise MonP4ShadowError("Full candidate is not in built_unapproved state")
    if variant_manifest.get("availability") is not True:
        raise MonP4ShadowError("Full candidate is not available")
    if variant_manifest.get("production_promoted") is not False:
        raise MonP4ShadowError("Full candidate has already been promoted")
    if variant_manifest.get("training_contaminated_descriptive_only") is not True:
        raise MonP4ShadowError("Full candidate descriptive-only contract changed")
    if list(variant_manifest.get("evaluation_sessions_allowed") or []):
        raise MonP4ShadowError("Full candidate unexpectedly allows a training-session evaluation")
    ...
    if not candidate_variant_id or candidate_embedding_records <= 0:
        raise MonP4ShadowError("Full-candidate variant ID or embedding record count is invalid")
    if not family_content_fingerprint:
        raise MonP4ShadowError("Candidate family content fingerprint is missing")
    recorded_hash = _text(
    ...
    if not recorded_hash or recorded_hash != actual_hash:
        raise MonP4ShadowError("Full-candidate embedding hash does not match its manifest")
...
def _validate_timetable(timetable: Path, slot_id: str, subject_abbr: str) -> None:
    try:
        ...
        raise MonP4ShadowError(f"Could not read timetable: {exc}") from exc
    if "slot_id" not in frame.columns:
        raise MonP4ShadowError("Timetable is missing slot_id")
    rows = frame[frame["slot_id"].astype(str).str.upper().eq(slot_id.upper())]
    if len(rows) != 1:
        raise MonP4ShadowError(f"Timetable must contain exactly one {slot_id} row; found {len(rows)}")
    row = rows.iloc[0]
    if _text(row.get("period")) not in {"4", "P4"}:
        raise MonP4ShadowError(f"{slot_id} is not period P4")
    if _text(row.get("section")).upper() != "B51":
        raise MonP4ShadowError(f"{slot_id} is not section B51")
    abbrs = {_text(value).upper() for value in _text(row.get("course_abbr")).replace("/", ",").split(",")}
    if subject_abbr.upper() not in abbrs:
        raise MonP4ShadowError(f"{slot_id} does not include subject {subject_abbr}")
...
def _resolve_camera_zones(repo_root: Path, reference_configuration: dict[str, Any]) -> Path:
    ...
    if not camera_zones.is_file():
        raise MonP4ShadowError(f"Reference camera-zone configuration not found: {camera_zones}")
    return camera_zones


def preflight_mon_p4_shadow(
    *,
    repo_root: Path,
    family_dir: Path,
    freeze_json: Path,
    ...
    for label, path in (
        ...
        if not path.is_file():
            raise MonP4ShadowError(f"Required {label} not found: {path}")
    ...
    if production_embeddings_sha256 != EXPECTED_PRODUCTION_EMBEDDINGS_SHA256:
        raise MonP4ShadowError(
            "Production embedding hash changed before MON_P4 shadow validation"
        )
    if production_summary_sha256 != EXPECTED_PRODUCTION_SUMMARY_SHA256:
        raise MonP4ShadowError("Production summary hash changed before MON_P4 shadow validation")
    try:
    ...
    if session_id != expected_session:
        raise MonP4ShadowError(f"Session ID must resolve exactly to {expected_session}")
    if session_fields["session_date"] != "2026-06-22" or session_fields["period"] != "P4":
        raise MonP4ShadowError("Phase 1.2N is restricted to untouched 2026-06-22 B51 P4 CVO")
    if slot_id != EXPECTED_SLOT_ID or subject_abbr.upper() != EXPECTED_SUBJECT:
        raise MonP4ShadowError("Phase 1.2N slot or subject changed from MON_P4/CVO")
    ...
    for video in videos:
        ...
        if frozen is None or frozen["sha256"] != video["sha256"] or frozen["length"] != video["file_size_bytes"]:
            raise MonP4ShadowError(f"Discovered MON_P4 video does not match source freeze: {relative}")
    ...
    run_id = f"mon-p4-shadow-{fingerprint_sha256[:20]}"
    output_dir = output_root / run_id
    diagnostics_root = repo_root / "attendance_output" / "diagnostics"
    production_diagnostic_run = diagnostics_root / f"{session_id}_PHASE_12N_PRODUCTION_{fingerprint_sha256[:12]}"
    candidate_diagnostic_run = diagnostics_root / f"{session_id}_PHASE_12N_CANDIDATE_{fingerprint_sha256[:12]}"
    return MonP4Preflight(
        repo_root=repo_root,
        session_id=session_id,
...
def _read_diagnostic_artifacts(
    ...
    if not diagnostic_run.is_dir():
        raise MonP4ShadowError(f"Diagnostic run not found: {diagnostic_run}")
    ...
    if _text(metadata.get("session_id")) != preflight.session_id:
        raise MonP4ShadowError("Diagnostic session ID does not match Phase 1.2N preflight")
    if _text(metadata.get("subject_abbr")).upper() != preflight.subject_abbr:
        raise MonP4ShadowError("Diagnostic subject does not match Phase 1.2N preflight")
    if _text(metadata.get("slot_id")).upper() != preflight.slot_id:
        raise MonP4ShadowError("Diagnostic slot does not match MON_P4")
    if _text(metadata.get("period")).upper() != "P4":
        raise MonP4ShadowError("Diagnostic period does not match P4")
    if metadata.get("diagnostic_only") is not True:
        raise MonP4ShadowError("Diagnostic run is not marked diagnostic_only=true")
    if metadata.get("official_attendance_written") is not False:
        raise MonP4ShadowError("Diagnostic run indicates official attendance was written")
    if dict(summary.get("normal_output_files") or {}):
        raise MonP4ShadowError("Diagnostic-only run unexpectedly contains normal attendance outputs")
    if not summary.get("checkpoint_extraction_valid"):
        raise MonP4ShadowError("Diagnostic checkpoint extraction is invalid")
    ...
    for key in exact_keys:
        if _text(metadata.get(key)) != _text(reference.get(key)):
            raise MonP4ShadowError(f"Diagnostic setting {key} differs from frozen reference")
    ...
    for label, frame, required in (
        ...
        if missing:
            raise MonP4ShadowError(f"{label} are missing columns: {', '.join(missing)}")
    if not tracklets.empty and tracklets["Tracklet_ID"].duplicated().any():
        raise MonP4ShadowError("Diagnostic tracklets contain duplicate Tracklet_ID values")
    if not observations.empty and observations["Observation_ID"].duplicated().any():
        raise MonP4ShadowError("Diagnostic observations contain duplicate Observation_ID values")
...
def _build_face_comparison(production: pd.DataFrame, candidate: pd.DataFrame) -> pd.DataFrame:
    ...
    if left["Face_Key"].duplicated().any() or right["Face_Key"].duplicated().any():
        raise MonP4ShadowError("Face diagnostics contain duplicate stable geometry keys")
    _assert_same_structure(
        left,
        right,
        key="Face_Key",
...
def snapshot_protected_state(preflight: MonP4Preflight) -> dict[str, Any]:
    return {
        "schema_version": SCHEMA_VERSION,
...
def _write_preflight_json(preflight: MonP4Preflight, output_path: Path) -> Path:
...
def _copy_input(path: Path, destination_dir: Path) -> Path:
    ...
    if _sha256_file(destination) != _sha256_file(path):
        raise MonP4ShadowError(f"Copied input hash mismatch: {path}")
    return destination


def _diagnostic_artifact_hashes(
    production: DiagnosticArtifacts,
    candidate: DiagnosticArtifacts,
) -> dict[str, str]:
    hashes: dict[str, str] = {}
    for label, artifacts in (("production", production), ("candidate", candidate)):
        for artifact_label, path in (
            ("summary", artifacts.summary_path),
            ("faces", artifacts.face_path),
            ("tracklets", artifacts.tracklet_path),
            ("observations", artifacts.observations_path),
        ):
            hashes[f"{label}/{artifact_label}/{path.name}"] = _sha256_file(path)
    return hashes
...
def _prepare_review_rows(
    ...
    fields = {
        "Phase_1_2N_Delta_Type": "Tracklet_Delta_Type",
        "Phase_1_2N_Has_Tracklet_Delta": "Has_Tracklet_Delta",
        "Phase_1_2N_Has_Observation_Frame_Delta": "Has_Observation_Frame_Delta",
        "Phase_1_2N_Has_Official_Frame_Delta": "Has_Official_Frame_Delta",
...
def _verify_completed_output(preflight: MonP4Preflight) -> MonP4RunResult | None:
    ...
    if not manifest.is_file() or not summary_path.is_file():
        raise MonP4ShadowError(
            f"Existing Phase 1.2N output is incomplete; do not overwrite it: {preflight.output_dir}"
        )
    try:
        verify_output_manifest(preflight.output_dir, manifest)
    except ShadowValidationError as exc:
        raise MonP4ShadowError(str(exc)) from exc
    summary = _load_json(summary_path, "Phase 1.2N summary")
    if _text(summary.get("fingerprint_sha256")) != preflight.fingerprint_sha256:
        raise MonP4ShadowError("Existing Phase 1.2N output fingerprint does not match the current inputs")
    review_package = Path(_text(summary.get("review_package")))
    return MonP4RunResult(
...
def run_mon_p4_shadow(
    ...
    *,
    ...
    if existing is not None:
        return existing
    diagnostic_paths = (
        preflight.production_diagnostic_run,
        preflight.candidate_diagnostic_run,
    )
    diagnostic_presence = [path.exists() for path in diagnostic_paths]
    if any(diagnostic_presence) and not all(diagnostic_presence):
        present = [str(path) for path, exists in zip(diagnostic_paths, diagnostic_presence) if exists]
        missing = [str(path) for path, exists in zip(diagnostic_paths, diagnostic_presence) if not exists]
        raise MonP4ShadowError(
            "Phase 1.2N has only one diagnostic side; refusing recognition and resume. "
            f"present={present}, missing={missing}"
        )
    resume_from_existing_diagnostics = all(diagnostic_presence)

    preflight.output_root.mkdir(parents=True, exist_ok=True)
    temporary_output = preflight.output_root / (
        f".{preflight.run_id}.tmp-{os.getpid()}-{time.time_ns()}"
    )
    if temporary_output.exists():
        raise MonP4ShadowError(f"Temporary Phase 1.2N output already exists: {temporary_output}")
    ...
    if resume_from_existing_diagnostics:
        if status_callback:
            status_callback(
                "Reusing both existing, strictly validated diagnostic-only MON_P4 runs after "
                "a post-processing failure. Recognition will not run again."
            )
    else:
        if status_callback:
            status_callback(
                "Running production diagnostic-only MON_P4 pass. Official attendance remains disabled."
            )
        _run_command(production_command, cwd=preflight.repo_root, status_callback=status_callback)
        if status_callback:
            status_callback(
                "Running candidate diagnostic-only MON_P4 pass. Candidate remains unpromoted."
            )
        _run_command(candidate_command, cwd=preflight.repo_root, status_callback=status_callback)

    production_artifacts = _read_diagnostic_artifacts(
        preflight.production_diagnostic_run, preflight
    )
    candidate_artifacts = _read_diagnostic_artifacts(
        preflight.candidate_diagnostic_run, preflight
    )
    diagnostic_hashes_before = _diagnostic_artifact_hashes(
        production_artifacts, candidate_artifacts
    )
    ...
    if not protected_comparison["unchanged"]:
        raise MonP4ShadowError(
            "Protected production/dataset/attendance state changed during Phase 1.2N: "
    ...
    try:
        review_package = export_review_package(
            ...
            context_padding=2.0,
            package_prefix="mon_p4_candidate_delta_review",
    ...
    if unmapped_frame_deltas:
        ...
        decision = "hold_insufficient_mon_p4_evidence"
    elif exception_count:
        decision = "pending_blind_exception_review"
    else:
        decision = "mon_p4_shadow_complete_no_material_deltas_pending_explicit_promotion"
    summary = {
        "schema_version": SCHEMA_VERSION,
        "policy_version": POLICY_VERSION,
        "created_at": datetime.now().isoformat(timespec="seconds"),
        ...
        "review_status": (
            ...
            if decision == "hold_insufficient_mon_p4_evidence"
        ...
        "candidate_promoted": False,
        "production_approved": False,
        "mon_p4_processed": True,
        "recognition_executed_in_this_invocation": not resume_from_existing_diagnostics,
        "recognition_reused_from_existing_diagnostics": resume_from_existing_diagnostics,
        "resumed_after_postprocessing_failure": resume_from_existing_diagnostics,
        "postprocessing_revision": POSTPROCESSING_REVISION,
        "diagnostic_artifacts_sha256": diagnostic_hashes_before,
        "canonical_source_only": True,
        "manual_review_scope": "exceptions_only",
        "broad_manual_review_required": False,
        "runtime_seconds": round(time.perf_counter() - started, 3),
        "exact_next_step": (
            "Complete the blind exception review and run Phase 1.2N evaluation."
            if decision == "pending_blind_exception_review"
            ...
            if decision == "hold_insufficient_mon_p4_evidence"
            else "Inspect the immutable Phase 1.2N report before any explicit promotion workflow."
        ),
    }
    ...
    try:
        ...
        raise MonP4ShadowError(str(exc)) from exc
    diagnostic_hashes_after = _diagnostic_artifact_hashes(
        production_artifacts, candidate_artifacts
    )
    if diagnostic_hashes_after != diagnostic_hashes_before:
        raise MonP4ShadowError(
            "Diagnostic evidence changed during Phase 1.2N post-processing; refusing finalization"
        )
    ...
    review_readme = (
        "PHASE 1.2N BLIND EXCEPTION REVIEW\n\n"
        ...
        "After exporting the completed labels CSV, run scripts/run_phase_1_2n_mon_p4_shadow.py evaluate "
        "--output-dir <PHASE_1_2N_OUTPUT> --labels <COMPLETED_LABELS.csv>.\n"
    ...
    launcher_path = preflight.output_dir / "open_mon_p4_exception_reviewer.ps1"
    launcher_command = (
...
def verify_mon_p4_output(
    *,
    ...
    if not output_dir.is_dir():
        raise MonP4ShadowError(f"Phase 1.2N output not found: {output_dir}")
    try:
        verified = verify_output_manifest(output_dir, output_dir / "output_manifest.json")
    except ShadowValidationError as exc:
        raise MonP4ShadowError(str(exc)) from exc
    summary = _load_json(output_dir / "shadow_run_summary.json", "Phase 1.2N summary")
    ...
    if protected.get("unchanged") is not True:
        raise MonP4ShadowError("Phase 1.2N output records a protected-state change")
    if _sha256_file(Path(production_embeddings)) != EXPECTED_PRODUCTION_EMBEDDINGS_SHA256:
        raise MonP4ShadowError("Current production embeddings differ from Phase 1.2N baseline")
    if _sha256_file(Path(production_summary)) != EXPECTED_PRODUCTION_SUMMARY_SHA256:
        raise MonP4ShadowError("Current production summary differs from Phase 1.2N baseline")
    family = verify_candidate_family(
        family_dir=family_dir,
        production_embeddings=production_embeddings,
        production_summary=production_summary,
    ...
    if family.candidate_variant_id != _text(summary.get("candidate_variant_id")):
        raise MonP4ShadowError("Phase 1.2N candidate variant changed")
    if summary.get("production_embeddings_changed") is not False:
        raise MonP4ShadowError("Phase 1.2N summary claims production embeddings changed")
    if summary.get("official_attendance_changed") is not False:
        raise MonP4ShadowError("Phase 1.2N summary claims official attendance changed")
    if summary.get("candidate_promoted") is not False:
        raise MonP4ShadowError("Phase 1.2N summary claims the candidate was promoted")
    return {
...
def evaluate_mon_p4_review(
    *,
    ...
    try:
        ...
        raise MonP4ShadowError(str(exc)) from exc
    if _text(metadata.get("phase")) != "1.2N" or _text(metadata.get("policy_version")) != POLICY_VERSION:
        raise MonP4ShadowError("Review package is not a Phase 1.2N exception package")
    if _text(metadata.get("fingerprint_sha256")) != _text(summary.get("fingerprint_sha256")):
        raise MonP4ShadowError("Review package fingerprint does not match the Phase 1.2N run")
    if not Path(labels_path).is_file():
        raise MonP4ShadowError(f"Completed Phase 1.2N labels not found: {labels_path}")
    ...
    if len(validated) != len(hidden):
        raise MonP4ShadowError("Phase 1.2N labels must contain every review item exactly once")
    if validated["Review_Status"].eq("").any():
        pending = validated.loc[validated["Review_Status"].eq(""), "Review_ID"].tolist()
        raise MonP4ShadowError(
            "Every Phase 1.2N exception must be reviewed before evaluation; pending: "
            + ", ".join(pending[:10])
        )
    ...
    joined["Phase_1_2N_Result"] = result_codes
    counts = joined["Phase_1_2N_Result"].value_counts().sort_index().to_dict()
    candidate_false = int(
        joined["Phase_1_2N_Result"].isin(
            {"candidate_false_identity", "candidate_unsafe_nonstudent_or_mixed"}
        ).sum()
    )
    retention_losses = int(
        joined["Phase_1_2N_Result"].eq("candidate_lost_correct_production_accept").sum()
    )
    unverifiable = int(joined["Phase_1_2N_Result"].eq("unverifiable").sum())
    correct_recoveries = int(
        joined["Phase_1_2N_Result"].eq("candidate_correct_recovery").sum()
    )
    corrected_identities = int(
        joined["Phase_1_2N_Result"].eq("candidate_corrected_production_identity").sum()
    )
    if candidate_false:
        decision = "mon_p4_shadow_failed_false_identity"
    elif retention_losses:
        decision = "mon_p4_shadow_failed_retention"
    elif unverifiable:
        decision = "mon_p4_shadow_hold_unverifiable_exceptions"
    else:
        decision = "mon_p4_shadow_passed_pending_explicit_promotion"
    evaluation_id = "mon-p4-evaluation-" + _stable_digest(
        {
    ...
    if evaluation_dir.exists():
        try:
            ...
            raise MonP4ShadowError(str(exc)) from exc
        existing = _load_json(
            evaluation_dir / "mon_p4_evaluation_summary.json", "existing Phase 1.2N evaluation"
        )
    ...
    evaluation_summary = {
        ...
        "exact_next_step": (
            ...
            if decision == "mon_p4_shadow_passed_pending_explicit_promotion"
            else "Keep the candidate unpromoted and inspect the failed or held exception evidence."
        ),
    }
    (evaluation_dir / "mon_p4_evaluation_summary.json").write_text(
        json.dumps(json_safe(evaluation_summary), indent=2), encoding="utf-8"
    )
    report = (
        "# Phase 1.2N MON_P4 Shadow Evaluation\n\n"
        f"- Decision: {decision}\n"
    ...
    (evaluation_dir / "mon_p4_evaluation_report.md").write_text(report, encoding="utf-8")
    write_output_manifest(
        evaluation_dir,
        {
            "policy_version": POLICY_VERSION,
...
def write_preflight(preflight: MonP4Preflight, output_path: Path) -> Path:
