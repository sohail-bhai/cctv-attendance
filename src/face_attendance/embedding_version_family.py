...
POLICY_VERSION = "phase-1.2i-b-v1.2"
...
REQUIRED_CCTV_EMBEDDING_SESSIONS = (TUE_P1_SESSION, TUE_P2_SESSION)
MON_P3_TOKEN = "MON_P3"
...
@dataclass(frozen=True)
class CctvSourceSelectionContract:
    selection_audit_id: str
    selection_policy_version: str
    source_ablation_id: str
    corrected_config_id: str
    kept_source_ids: tuple[str, ...]
    removed_source_ids: tuple[str, ...]
    selection_audit_output_manifest_sha256: str
    selection_decision_sha256: str
    source_ablation_output_manifest_sha256: str
    final_promotion_requires_new_untouched_session: bool = True

    def normalized(self) -> dict[str, Any]:
        kept = tuple(sorted({_text(value) for value in self.kept_source_ids if _text(value)}))
        removed = tuple(sorted({_text(value) for value in self.removed_source_ids if _text(value)}))
        if not _text(self.selection_audit_id):
            raise EmbeddingFamilyError("CCTV source selection has no audit ID")
        if not _text(self.selection_policy_version):
            raise EmbeddingFamilyError("CCTV source selection has no policy version")
        if not _text(self.source_ablation_id):
            raise EmbeddingFamilyError("CCTV source selection has no source-ablation ID")
        if not _text(self.corrected_config_id):
            raise EmbeddingFamilyError("CCTV source selection has no corrected configuration ID")
        if not kept or not removed:
            raise EmbeddingFamilyError(
                "CCTV source selection must keep at least one and remove at least one implicated source"
            )
        overlap = sorted(set(kept).intersection(removed))
        if overlap:
            raise EmbeddingFamilyError(
                "CCTV source selection keep/remove sets overlap: " + ", ".join(overlap)
            )
        for label, value in (
            ("selection_audit_output_manifest_sha256", self.selection_audit_output_manifest_sha256),
            ("selection_decision_sha256", self.selection_decision_sha256),
            ("source_ablation_output_manifest_sha256", self.source_ablation_output_manifest_sha256),
        ):
            token = _text(value).lower()
            if not re.fullmatch(r"[0-9a-f]{64}", token):
                raise EmbeddingFamilyError(f"CCTV source selection {label} is not SHA-256")
        if self.final_promotion_requires_new_untouched_session is not True:
            raise EmbeddingFamilyError(
                "CCTV source selection must require a new untouched final validation session"
            )
        return {
            "schema_version": FAMILY_SCHEMA_VERSION,
            "policy_version": SOURCE_SELECTED_FAMILY_POLICY_VERSION,
            "selection_audit_id": _text(self.selection_audit_id),
            "selection_policy_version": _text(self.selection_policy_version),
            "source_ablation_id": _text(self.source_ablation_id),
            "corrected_config_id": _text(self.corrected_config_id),
            "kept_source_ids": list(kept),
            "removed_source_ids": list(removed),
            "implicated_source_ids": sorted(set(kept).union(removed)),
            "selection_audit_output_manifest_sha256": _text(
                self.selection_audit_output_manifest_sha256
            ).lower(),
            "selection_decision_sha256": _text(self.selection_decision_sha256).lower(),
            "source_ablation_output_manifest_sha256": _text(
                self.source_ablation_output_manifest_sha256
            ).lower(),
...
VARIANT_BUILT_STATUS = "built_unapproved"
VARIANT_UNAVAILABLE_STATUS = "unavailable_no_usable_source_evidence"


def resolve_variant_availability(
    staging_manifest: dict[str, Any],
) -> dict[str, dict[str, Any]]:
    """Describe which leakage-safe variants can truthfully be built.

    Human approval and technical embedding usability are separate. A missing
    technically usable session must not weaken preprocessing, fabricate a
    cross-session variant, or abort the enrollment-only candidate family.
    """
    usable_counts = {
        _text(session): int(count)
        for session, count in dict(
            staging_manifest.get("embedding_usable_session_counts") or {}
        ).items()
    }
    p1_available = int(usable_counts.get(TUE_P1_SESSION, 0)) > 0
    p2_available = int(usable_counts.get(TUE_P2_SESSION, 0)) > 0
    actual_sessions = tuple(
        session
        for session in REQUIRED_CCTV_EMBEDDING_SESSIONS
        if int(usable_counts.get(session, 0)) > 0
    )
    ...
    def entry(
        *,
        ...
        required_sessions = tuple(required)
        included_sessions = tuple(included)
        missing = tuple(
            session for session in required_sessions if session not in included_sessions
        )
        return {
            "available": bool(available),
            "status": VARIANT_BUILT_STATUS if available else VARIANT_UNAVAILABLE_STATUS,
            "required_source_sessions": list(required_sessions),
            "source_sessions_included": list(included_sessions),
            "missing_usable_source_sessions": list(missing),
            "evaluation_sessions_allowed": list(evaluation if available else ()),
            "evaluation_sessions_contaminated": list(
                contaminated if available else ()
            ),
    ...
    plan = {
        "A": entry(
            ...
            required=(),
            included=(),
            evaluation=(TUE_P1_SESSION, TUE_P2_SESSION),
            contaminated=(),
        ),
        "B": entry(
            available=p2_available,
            required=(TUE_P2_SESSION,),
            included=(TUE_P2_SESSION,) if p2_available else (),
            evaluation=(TUE_P1_SESSION,),
            contaminated=(TUE_P2_SESSION,),
            reason="no_technically_usable_tue_p2_cctv_embedding_source",
        ),
        "C": entry(
            available=p1_available,
            required=(TUE_P1_SESSION,),
            included=(TUE_P1_SESSION,) if p1_available else (),
            evaluation=(TUE_P2_SESSION,),
            contaminated=(TUE_P1_SESSION,),
            reason="no_technically_usable_tue_p1_cctv_embedding_source",
        ),
        "D": entry(
            available=bool(actual_sessions),
            required=REQUIRED_CCTV_EMBEDDING_SESSIONS,
            included=actual_sessions,
            evaluation=(),
            contaminated=actual_sessions,
            reason="no_technically_usable_cctv_embedding_source",
        ),
    }
    plan["summary"] = {
        "usable_session_counts": usable_counts,
        "available_variants": [
            key for key in ("A", "B", "C", "D") if plan[key]["available"]
        ],
        "unavailable_variants": [
            key for key in ("A", "B", "C", "D") if not plan[key]["available"]
        ],
        "cross_session_pair_complete": bool(p1_available and p2_available),
        "partial_family_allowed": True,
...
def _effective_variant_definition(
    definition: VariantDefinition, availability: dict[str, Any]
) -> VariantDefinition:
    return VariantDefinition(
        definition.key,
        definition.directory,
        definition.role,
        tuple(availability.get("source_sessions_included") or ()),
        tuple(availability.get("evaluation_sessions_allowed") or ()),
        tuple(availability.get("evaluation_sessions_contaminated") or ()),
        definition.descriptive_only,
    )
...
def _review_scalability_policy() -> dict[str, Any]:
    """Permanent review-lifecycle contract for live CCTV scale-out."""
    return {
        "current_manual_reviews_are_one_time_bootstrap_ground_truth": True,
        "reviewed_tracks_reused_across_embedding_versions": True,
        "approval_and_rejection_decisions_reused_permanently": True,
        "future_embedding_updates_are_incremental_per_student": True,
        "every_embedding_update_requires_automatic_regression": True,
        "live_review_mode": "exception_only",
        "live_review_reasons": [
            "unknown_identity",
            "conflicting_identity_evidence",
            "low_confidence_or_low_quality_evidence",
        ],
        "live_identity_decision_basis": "multi_frame_tracklet_and_checkpoint_evidence",
        "review_work_must_not_scale_linearly_with_frames_or_cameras": True,
        "broad_review_round_requires_documented_exception": True,
        "post_mon_p3_priority": [
            "live_cctv_ingestion",
            "asynchronous_processing",
            "checkpoint_state",
            "prioritized_exception_review_queue",
            "hod_and_faculty_workflows",
        ],
    }
...
RECOVERY_FAMILY_POLICY_VERSION = "phase-1.2i-d-recovered-family-v1"
SOURCE_SELECTED_FAMILY_POLICY_VERSION = "phase-1.2m-source-selected-family-v1"
RECOVERY_CONSUMPTION_MIN_PAIRWISE_SIMILARITY = 0.40


@dataclass(frozen=True)
class RecoveryConsumptionPlan:
    recovery_dir: Path
...
def validate_same_track_recovery_for_family(
    *,
    recovery_dir: Path,
    versions_root: Path,
    ...
    expected_target_session: str = TUE_P2_SESSION,
    expected_target_tracks: int = 5,
    expected_selected_embeddings: int = 10,
    min_consumption_pairwise_similarity: float = RECOVERY_CONSUMPTION_MIN_PAIRWISE_SIMILARITY,
) -> RecoveryConsumptionPlan:
    """Validate a same-track recovery bundle for candidate-family consumption.

    Recovery preserves human-reviewed identity ownership, but candidate-family
    consumption is intentionally stricter than recovery generation: exactly one
    medoid embedding is consumed per reviewed track. The second recovered vector
    remains immutable audit evidence and is not used to overweight a track.
    """
    from .cctv_same_track_recovery import (  # Local import avoids a module cycle.
    ...
    if not recovery_dir.is_dir():
        raise EmbeddingFamilyError(f"Recovery bundle not found: {recovery_dir}")
    try:
        manifest = verify_same_track_recovery(recovery_dir)
    except SameTrackRecoveryError as exc:
        raise EmbeddingFamilyError(str(exc)) from exc

    if _text(manifest.get("status")) != "recovery_available":
        raise EmbeddingFamilyError(
            "Recovery bundle is not available for family consumption: "
            + _text(manifest.get("status"))
        )
    if _text(manifest.get("target_session")) != expected_target_session:
        raise EmbeddingFamilyError(
            "Recovery session mismatch: expected "
            f"{expected_target_session}, found {_text(manifest.get('target_session'))}"
        )
    if manifest.get("production_files_changed") is not False:
        raise EmbeddingFamilyError("Recovery bundle reports changed production files")
    if manifest.get("dataset_files_changed") is not False:
        raise EmbeddingFamilyError("Recovery bundle reports changed dataset files")
    if manifest.get("official_attendance_changed") is not False:
        raise EmbeddingFamilyError("Recovery bundle reports changed attendance")
    if manifest.get("candidate_promoted") is not False:
        raise EmbeddingFamilyError("Recovery bundle reports candidate promotion")
    if manifest.get("mon_p3_processed") is not False:
        raise EmbeddingFamilyError("Recovery bundle reports MON_P3 processing")
    if manifest.get("manual_review_requested") is not False:
        raise EmbeddingFamilyError("Recovery bundle unexpectedly requests manual review")

    current_embeddings_hash = _sha256_file(production_embeddings_path)
    current_summary_hash = _sha256_file(production_summary_path)
    for field, current in (
        ("production_embeddings_before_sha256", current_embeddings_hash),
        ("production_embeddings_after_sha256", current_embeddings_hash),
        ("production_summary_before_sha256", current_summary_hash),
        ("production_summary_after_sha256", current_summary_hash),
    ):
        if _text(manifest.get(field)) != current:
            raise EmbeddingFamilyError(
                f"Recovery bundle production hash contract failed: {field}"
            )

    source_family_id = _text(manifest.get("source_family_id"))
    if not source_family_id:
        raise EmbeddingFamilyError("Recovery bundle has no source family ID")
    source_family_dir = versions_root / source_family_id
    source_verification = verify_embedding_family(
        source_family_dir,
        production_embeddings_path=production_embeddings_path,
        production_summary_path=production_summary_path,
    )
    if _text(source_verification.get("decision")) != _text(
        manifest.get("source_family_decision")
    ):
        raise EmbeddingFamilyError("Recovery source-family decision changed")
    expected_source_manifest_hash = _text(
        dict(manifest.get("input_fingerprint") or {}).get(
            "family_output_manifest_sha256"
        )
    )
    if not expected_source_manifest_hash or _sha256_file(
        source_family_dir / "output_manifest.json"
    ) != expected_source_manifest_hash:
        raise EmbeddingFamilyError("Recovery source-family manifest hash changed")

    usability_path = source_family_dir / "cctv_embedding_usability.csv"
    if not usability_path.is_file():
        raise EmbeddingFamilyError(
            f"Recovery source family lacks CCTV usability audit: {usability_path}"
        )
    usability = pd.read_csv(usability_path, dtype=str, keep_default_na=False)
    required_columns = {
        "Item_ID",
        "Track_ID",
        "Observation_ID",
    ...
    missing = sorted(required_columns.difference(usability.columns))
    if missing:
        raise EmbeddingFamilyError(
            "Recovery source-family usability audit is missing columns: "
            + ", ".join(missing)
        )
    original_unusable = usability[
        usability["Source_Session"].eq(expected_target_session)
        & ~usability["Embedding_Usable"].map(_yes)
        & usability["Item_ID"].astype(str).str.startswith("CCC-")
    ].copy()
    target_items = list(manifest.get("input_fingerprint", {}).get("target_items") or [])
    expected_item_ids = {_text(row.get("item_id")) for row in target_items}
    source_item_ids = set(original_unusable["Item_ID"].map(_text))
    if expected_item_ids != source_item_ids:
        raise EmbeddingFamilyError(
            "Recovery target items no longer match the source-family unusable P2 items"
        )
    if len(expected_item_ids) != expected_target_tracks:
        raise EmbeddingFamilyError(
            f"Expected {expected_target_tracks} recovered target tracks, found "
            f"{len(expected_item_ids)}"
        )

    audit_path = recovery_dir / "same_track_recovery_audit.csv"
    if not audit_path.is_file():
        raise EmbeddingFamilyError(f"Recovery audit missing: {audit_path}")
    ...
    audit_required = {
        ...
        "Super_Resolution_Used",
        "Audit_Crop_SHA256",
        "Detector_Score",
        "Matched_IoU",
    }
    missing = sorted(audit_required.difference(audit.columns))
    if missing:
        raise EmbeddingFamilyError(
            "Recovery audit is missing columns: " + ", ".join(missing)
        )
    selected_audit = audit[audit["Selected_For_Recovery"].map(_yes)].copy()
    if len(selected_audit) != expected_selected_embeddings:
        raise EmbeddingFamilyError(
            f"Expected {expected_selected_embeddings} selected recovery embeddings, "
            f"found {len(selected_audit)}"
        )
    if selected_audit["Embedding_Key"].duplicated().any():
        raise EmbeddingFamilyError("Recovery audit contains duplicate embedding keys")
    if not selected_audit["Identity_Source"].eq(
        "human_reviewed_track_actual_roll"
    ).all():
        raise EmbeddingFamilyError("Recovery identity source is not human-reviewed track truth")
    for column in (
        "Model_Prediction_Used_As_Identity",
        "Manual_Rereview_Required",
        "Quality_Gate_Relaxed",
        ...
        if selected_audit[column].map(_yes).any():
            raise EmbeddingFamilyError(
                f"Recovery safety flag is unexpectedly enabled: {column}"
            )

    selected_manifest = list(manifest.get("selected_recovered_embeddings") or [])
    manifest_keys = {_text(row.get("embedding_key")) for row in selected_manifest}
    audit_keys = set(selected_audit["Embedding_Key"].map(_text))
    if manifest_keys != audit_keys:
        raise EmbeddingFamilyError(
            "Recovery manifest and audit selected embedding sets do not match"
        )
    track_results = list(manifest.get("track_results") or [])
    if len(track_results) != expected_target_tracks:
        raise EmbeddingFamilyError(
            f"Expected {expected_target_tracks} recovery track results, found "
            f"{len(track_results)}"
        )
    metadata_by_key = {
        _text(row.get("embedding_key")): dict(row) for row in selected_manifest
    }
    audit_by_key = {
        _text(row["Embedding_Key"]): row for row in selected_audit.to_dict("records")
    }

    embeddings_path = recovery_dir / "recovered_embeddings.npz"
    consumed_sources: list[dict[str, Any]] = []
    with np.load(embeddings_path, allow_pickle=False) as payload:
        for result in track_results:
            if _text(result.get("status")) != "recovered":
                raise EmbeddingFamilyError(
                    "Recovery track is not fully recovered: "
                    + _text(result.get("track_id"))
                )
            pairwise_min = result.get("selected_pairwise_min")
            if pairwise_min is None or float(pairwise_min) < float(
                min_consumption_pairwise_similarity
            ):
                raise EmbeddingFamilyError(
                    "Recovered track does not meet the stricter family-consumption "
                    f"similarity floor {min_consumption_pairwise_similarity:.2f}: "
                    f"{_text(result.get('track_id'))}"
                )
            medoid_observation_id = _text(result.get("medoid_observation_id"))
            selected_observation_ids = {
                _text(value) for value in result.get("selected_observation_ids") or []
            }
            if not medoid_observation_id or medoid_observation_id not in selected_observation_ids:
                raise EmbeddingFamilyError("Recovery track medoid is missing from selected evidence")
            matches = [
                row
                for row in selected_manifest
                if _text(row.get("item_id")) == _text(result.get("item_id"))
                and _text(row.get("track_id")) == _text(result.get("track_id"))
                and _text(row.get("observation_id")) == medoid_observation_id
            ]
            if len(matches) != 1:
                raise EmbeddingFamilyError(
                    "Recovery track does not resolve to exactly one selected medoid"
                )
            metadata = dict(matches[0])
            embedding_key = _text(metadata.get("embedding_key"))
            if embedding_key not in payload.files:
                raise EmbeddingFamilyError(
                    f"Recovery embedding payload is missing medoid key: {embedding_key}"
                )
            vector = np.asarray(payload[embedding_key], dtype=np.float32).reshape(-1)
            if vector.size != 128 or not np.isfinite(vector).all():
                raise EmbeddingFamilyError(
                    f"Recovery medoid embedding is invalid: {embedding_key}"
                )
            norm = float(np.linalg.norm(vector))
            if norm <= 1e-12:
                raise EmbeddingFamilyError(
                    f"Recovery medoid embedding has zero norm: {embedding_key}"
                )
            vector = vector / norm
            candidate_roll = _canonical(metadata.get("candidate_roll"))
            if candidate_roll not in roster_rolls:
                raise EmbeddingFamilyError(
                    f"Recovered identity is not in the authoritative CVO roster: {candidate_roll}"
                )
            if _text(metadata.get("source_session")) != expected_target_session:
                raise EmbeddingFamilyError("Recovered medoid source session changed")
            if _text(metadata.get("identity_source")) != "human_reviewed_track_actual_roll":
                raise EmbeddingFamilyError("Recovered medoid identity source changed")
            for field in (
                "model_prediction_used_as_identity",
                "manual_rereview_required",
                "quality_gate_relaxed",
                ...
                if metadata.get(field) is not False:
                    raise EmbeddingFamilyError(
                        f"Recovered medoid safety contract changed: {field}"
                    )
            audit_row = audit_by_key.get(embedding_key)
            if audit_row is None:
                raise EmbeddingFamilyError(
                    f"Recovery medoid is absent from the audit CSV: {embedding_key}"
                )
            for column, expected in (
                ("Item_ID", _text(metadata.get("item_id"))),
                ("Track_ID", _text(metadata.get("track_id"))),
                ("Observation_ID", medoid_observation_id),
                ("Candidate_Roll", candidate_roll),
                ("Source_Session", expected_target_session),
            ):
                if _text(audit_row.get(column)) != expected:
                    raise EmbeddingFamilyError(
                        f"Recovery medoid audit mismatch for {embedding_key}: {column}"
                    )
            if float(audit_row.get("Matched_IoU") or 0.0) < 0.35:
                raise EmbeddingFamilyError("Recovered medoid IoU is below the frozen policy")
            if float(audit_row.get("Detector_Score") or 0.0) < 0.85:
                raise EmbeddingFamilyError(
                    "Recovered medoid detector score is below the frozen policy"
                )
            crop_path = recovery_dir / "recovered_crops" / f"{embedding_key}.png"
            crop_sha256 = _text(audit_row.get("Audit_Crop_SHA256"))
            if not crop_path.is_file() or not crop_sha256:
                raise EmbeddingFamilyError(
                    f"Recovered medoid audit crop is missing: {crop_path}"
                )
            if _sha256_file(crop_path) != crop_sha256:
                raise EmbeddingFamilyError(
                    f"Recovered medoid audit crop hash changed: {embedding_key}"
                )
            original = original_unusable[
                original_unusable["Item_ID"].eq(_text(metadata.get("item_id")))
            ]
            if len(original) != 1:
                raise EmbeddingFamilyError(
                    "Recovered medoid cannot be tied to exactly one original approved item"
                )
            original_row = original.iloc[0]
            if (
                _text(original_row["Track_ID"]) != _text(metadata.get("track_id"))
                or _text(original_row["Candidate_Roll"]) != candidate_roll
            ):
                raise EmbeddingFamilyError(
                    "Recovered medoid ownership changed from the source-family approval"
                )
            consumed_sources.append(
                {
                    "source_id": (
                        f"RECOVERY-{_text(manifest.get('recovery_id'))}-"
                        f"{_text(metadata.get('item_id'))}"
                    ),
                    ...
                    "candidate_roll": candidate_roll,
                    "source_session": expected_target_session,
                    "checkpoint": _text(metadata.get("checkpoint")),
                    "camera": _text(metadata.get("camera")),
                    "frame_index": int(metadata.get("frame_index") or 0),
                    "video_path": _text(metadata.get("video_path")),
                    "video_sha256": _text(metadata.get("video_sha256")),
                    "detector_score": float(metadata.get("detector_score") or 0.0),
                    "matched_iou": float(metadata.get("matched_iou") or 0.0),
                    "selected_pairwise_min": float(pairwise_min),
                    "crop_path": crop_path,
    ...
    if len(consumed_sources) != expected_target_tracks:
        raise EmbeddingFamilyError(
            f"Expected one consumed medoid per track ({expected_target_tracks}), "
            f"found {len(consumed_sources)}"
        )
    source_ids = [row["source_id"] for row in consumed_sources]
    if len(source_ids) != len(set(source_ids)):
        raise EmbeddingFamilyError("Recovery medoid source IDs are not unique")
    vector_hash_owners: dict[str, str] = {}
    for row in consumed_sources:
        digest = hashlib.sha256(
            np.asarray(row["embedding"], dtype=np.float32).tobytes()
        ).hexdigest()
        previous = vector_hash_owners.get(digest)
        if previous is not None and previous != row["candidate_roll"]:
            raise EmbeddingFamilyError(
                "Identical recovered embedding vectors are owned by different students"
            )
        vector_hash_owners[digest] = row["candidate_roll"]

    summary = {
        "schema_version": FAMILY_SCHEMA_VERSION,
        "policy_version": RECOVERY_FAMILY_POLICY_VERSION,
        "recovery_id": _text(manifest.get("recovery_id")),
        "recovery_dir": str(recovery_dir),
        "recovery_output_manifest_sha256": _sha256_file(
            recovery_dir / "output_manifest.json"
        ),
        "recovery_manifest_sha256": _sha256_file(
            recovery_dir / "recovery_manifest.json"
        ),
        "recovered_embeddings_sha256": _sha256_file(embeddings_path),
        "same_track_recovery_audit_sha256": _sha256_file(audit_path),
        "source_family_id": source_family_id,
        "source_family_output_manifest_sha256": expected_source_manifest_hash,
        "target_session": expected_target_session,
        "target_tracks": expected_target_tracks,
        "recovery_selected_embeddings_preserved": expected_selected_embeddings,
        "candidate_consumed_medoid_embeddings": len(consumed_sources),
        "consumption_rule": "exactly_one_medoid_embedding_per_human_reviewed_track",
        "minimum_selected_pairwise_similarity": float(
            min(row["selected_pairwise_min"] for row in consumed_sources)
        ),
        "minimum_consumption_pairwise_similarity": float(
            min_consumption_pairwise_similarity
        ),
        "recovered_identities": sorted(
            {row["candidate_roll"] for row in consumed_sources}
        ),
        "human_review_labels_reused": True,
    ...
    return RecoveryConsumptionPlan(
...
def stage_recovery_consumption_plan(
    *,
    plan: RecoveryConsumptionPlan,
    temporary_family_dir: Path,
    final_family_dir: Path,
    repo_root: Path,
) -> tuple[pd.DataFrame, list[dict[str, Any]], dict[str, Any]]:
    """Copy immutable recovery inputs and stage one medoid source per track."""
    temporary_family_dir = Path(temporary_family_dir)
    final_family_dir = Path(final_family_dir)
    repo_root = Path(repo_root).resolve()
    recovery_id = _text(plan.summary.get("recovery_id"))
    input_root = temporary_family_dir / "recovery_inputs" / recovery_id
    input_root.mkdir(parents=True, exist_ok=False)
    for name in (
        "recovery_manifest.json",
        "same_track_recovery_audit.csv",
        "recovered_embeddings.npz",
        "output_manifest.json",
    ):
        source = plan.recovery_dir / name
        destination = input_root / name
        shutil.copy2(source, destination)
        if _sha256_file(source) != _sha256_file(destination):
            raise EmbeddingFamilyError(
                f"Recovery input copy hash mismatch: {name}"
            )

    staging_root = (
        temporary_family_dir
        / "candidate_sources"
        / "same_track_recovered_cctv"
        / "tue_p2"
    )
    rows: list[dict[str, Any]] = []
    records: list[dict[str, Any]] = []
    for source in plan.sources:
        item_id = _text(source["item_id"])
        observation_token = re.sub(
            r"[^A-Za-z0-9._-]+", "_", _text(source["observation_id"])
        )
        destination = staging_root / f"{item_id}__{observation_token}.png"
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(Path(source["crop_path"]), destination)
        if _sha256_file(destination) != _text(source["crop_sha256"]):
            raise EmbeddingFamilyError(
                f"Staged recovery crop hash mismatch: {source['embedding_key']}"
            )
        final_destination = final_family_dir / destination.relative_to(
            temporary_family_dir
        )
        try:
        ...
        if image is None or image.size == 0:
            raise EmbeddingFamilyError(
                f"Staged recovery crop is unreadable: {destination}"
            )
        height, width = image.shape[:2]
        records.append(
            {
                "roll_no": _canonical(source["candidate_roll"]),
                "image_path": image_path,
                "detection_score": float(source["detector_score"]),
                "embedding": np.asarray(source["embedding"], dtype=np.float32),
                "_source_id": _text(source["source_id"]),
                "_source_kind": "same_track_recovered_cctv_medoid",
                "_source_session": _text(source["source_session"]),
            }
        )
        rows.append(
            {
                ...
                "Item_ID": item_id,
                "Source_Package_ID": recovery_id,
                "Source_Broad_Item_ID": item_id,
                "Source_Path": str(source["crop_path"]),
                "Source_SHA256": _text(source["crop_sha256"]),
                "Staged_Path": str(final_destination),
                "Staged_SHA256": _text(source["crop_sha256"]),
                "Source_Session": _text(source["source_session"]),
                "Track_ID": _text(source["track_id"]),
                "Observation_ID": _text(source["observation_id"]),
                "Checkpoint": _text(source["checkpoint"]),
                "Camera": _text(source["camera"]),
                "Human_Actual_Roll": _canonical(source["candidate_roll"]),
                "Candidate_Roll": _canonical(source["candidate_roll"]),
                "Identity_Source": "human_reviewed_track_actual_roll",
                "Identity_Correction_ID": "",
                "Human_Review_Action": "same_track_recovery_from_approved_item",
                "Human_Approval_Preserved": True,
                "Embedding_Usable": True,
                "Embedding_Status": "same_track_recovered_medoid",
                "Embedding_Exclusion_Reason": "",
                "Manual_Rereview_Required": False,
                "Quality_Gate_Relaxed": False,
                "Image_Width": int(width),
                "Image_Height": int(height),
                "Detector_Score": float(source["detector_score"]),
                "Rank_Score": "",
                "Detection_Count": 1,
                "Accepted_Candidate_Count": 1,
                "Preprocessing_Policy": "live_zone_same_track_exact_observation_reproduction",
                "Embedding_Dimension": int(
                    np.asarray(source["embedding"]).reshape(-1).size
                ),
                "Recovery_ID": recovery_id,
                "Recovery_Embedding_Key": _text(source["embedding_key"]),
                "Recovery_Selected_Pairwise_Min": float(
                    source["selected_pairwise_min"]
                ),
                "Recovery_Consumption_Role": "track_medoid_only",
                "Video_Path": _text(source["video_path"]),
                "Video_SHA256": _text(source["video_sha256"]),
                "Frame_Index": int(source["frame_index"]),
                "Matched_IoU": float(source["matched_iou"]),
                "Source_Category": "same_track_recovery",
            }
        )
    ...
    consumption_manifest = {
        **plan.summary,
        "staged_sources": rows,
        "staged_source_count": len(rows),
        "candidate_embedding_record_count": len(records),
    ...
    _write_csv(
        temporary_family_dir / "recovery_consumption_sources.csv", staging
    )
    _write_json(
        temporary_family_dir / "recovery_consumption_manifest.json",
        consumption_manifest,
    )
    return staging, records, consumption_manifest


def merge_recovery_into_cctv_staging(
    *,
    ...
    combined_staging = pd.concat(
        [original_staging, recovery_staging], ignore_index=True, sort=False
    )
    combined_records = list(original_records) + list(recovery_records)
    source_ids = [_text(record.get("_source_id")) for record in combined_records]
    if len(source_ids) != len(set(source_ids)):
        raise EmbeddingFamilyError("Original and recovered CCTV source IDs collide")
    usable_counts = {
        _text(key): int(value)
        for key, value in combined_staging.loc[
            combined_staging["Embedding_Usable"].map(bool), "Source_Session"
        ]
        .value_counts()
        .sort_index()
        .items()
    ...
    missing_sessions = [
        session
        for session in REQUIRED_CCTV_EMBEDDING_SESSIONS
        if int(usable_counts.get(session, 0)) < 1
    ]
    merged_manifest = dict(original_manifest)
    merged_manifest.update(
        {
            "staged_unique_crops": len(combined_staging),
            "embedding_usable_crops": len(combined_records),
            "approved_but_embedding_unusable_crops": int(
                (~original_staging["Embedding_Usable"].map(bool)).sum()
            ),
            "same_track_recovered_medoid_crops": len(recovery_staging),
            "same_track_recovery_id": _text(
                recovery_manifest.get("recovery_id")
            ),
            "embedding_usable_session_counts": usable_counts,
            "minimum_usable_evidence_passed": not missing_sessions,
            "missing_usable_sessions": missing_sessions,
            "recovery_consumption": recovery_manifest,
            "records": combined_staging.to_dict("records"),
            "manual_rereview_required": False,
            "quality_gate_relaxed": False,
            "super_resolution_used": False,
        }
    )
    merged_manifest["minimum_usable_evidence_policy"] = {
        **dict(original_manifest.get("minimum_usable_evidence_policy") or {}),
        "same_track_recovery_allowed": True,
        "same_track_recovery_consumption": "one_medoid_per_reviewed_track",
        "minimum_recovery_pairwise_similarity": (
            RECOVERY_CONSUMPTION_MIN_PAIRWISE_SIMILARITY
        ),
        "manual_rereview_required": False,
        "quality_gate_relaxed": False,
        "super_resolution_used": False,
    ...
    combined_records.sort(
        key=lambda record: (
            _text(record.get("_source_session")),
            _canonical(record.get("roll_no")),
            _text(record.get("_source_id")),
        )
    )
    return combined_staging, combined_records, merged_manifest


def apply_cctv_source_selection(
    *,
    ...
    """Apply an immutable reviewed source-subset decision after full audit staging.

    All approved and recovered inputs remain staged and auditable. Only embedding
    records explicitly removed by the reviewed selection contract are excluded
    from candidate variants. Unrelated CCTV sources remain active.
    """
    normalized = contract.normalized()
    kept = set(normalized["kept_source_ids"])
    removed = set(normalized["removed_source_ids"])
    implicated = kept | removed
    record_by_id: dict[str, dict[str, Any]] = {}
    for record in records:
        source_id = _text(record.get("_source_id"))
        if not source_id or source_id in record_by_id:
            raise EmbeddingFamilyError(
                "CCTV source selection requires unique non-empty source IDs"
            )
        record_by_id[source_id] = record
    missing = sorted(implicated.difference(record_by_id))
    if missing:
        raise EmbeddingFamilyError(
            "CCTV source selection references unavailable embedding sources: "
            + ", ".join(missing)
        )
    if len(implicated) != len(normalized["implicated_source_ids"]):
        raise EmbeddingFamilyError("CCTV source selection implicated-source count changed")

    filtered_records = [
        record for record in records if _text(record.get("_source_id")) not in removed
    ]
    active_ids = {_text(record.get("_source_id")) for record in filtered_records}
    if not kept.issubset(active_ids) or removed.intersection(active_ids):
        raise EmbeddingFamilyError("CCTV source selection was not applied exactly")
    if len(filtered_records) != len(records) - len(removed):
        raise EmbeddingFamilyError("CCTV source selection removed an unexpected record count")

    selected_staging = staging.copy()
    if "Source_ID" not in selected_staging.columns:
        raise EmbeddingFamilyError("CCTV staging audit has no Source_ID column")
    selected_staging["Source_Selection_Audit_ID"] = normalized["selection_audit_id"]
    selected_staging["Source_Selection_Config_ID"] = normalized["corrected_config_id"]
    selected_staging["Source_Selection_Implicated"] = selected_staging[
        "Source_ID"
    ].map(lambda value: _text(value) in implicated)
    selected_staging["Selected_For_Candidate"] = selected_staging[
        "Source_ID"
    ].map(lambda value: _text(value) in active_ids)

    def decision(source_id: Any) -> str:
        token = _text(source_id)
        if token in kept:
            return "kept_by_phase_1_2l_1_selection"
        if token in removed:
            return "removed_by_phase_1_2l_1_selection"
        if token in active_ids:
            return "not_implicated_kept"
        return "embedding_unusable_not_active"

    selected_staging["Source_Selection_Decision"] = selected_staging[
        "Source_ID"
    ].map(decision)
    active_counts: dict[str, int] = {}
    active_kind_counts: dict[str, int] = {}
    for record in filtered_records:
        session = _text(record.get("_source_session"))
        kind = _text(record.get("_source_kind"))
        active_counts[session] = active_counts.get(session, 0) + 1
        active_kind_counts[kind] = active_kind_counts.get(kind, 0) + 1
    active_counts = dict(sorted(active_counts.items()))
    active_kind_counts = dict(sorted(active_kind_counts.items()))
    missing_sessions = [
        session
        for session in REQUIRED_CCTV_EMBEDDING_SESSIONS
        if int(active_counts.get(session, 0)) < 1
    ]
    audit_columns = [
        "Source_ID",
        "Candidate_Roll",
        "Source_Session",
        "Embedding_Usable",
        "Source_Selection_Implicated",
        "Selected_For_Candidate",
        "Source_Selection_Decision",
        "Source_Selection_Audit_ID",
        "Source_Selection_Config_ID",
    ]
    audit = selected_staging[
        [column for column in audit_columns if column in selected_staging.columns]
    ].copy()
    audit = audit.sort_values("Source_ID", kind="stable").reset_index(drop=True)
    summary = {
        **normalized,
        "status": "applied",
        "pre_selection_usable_cctv_embedding_records": len(records),
        "active_cctv_embedding_records": len(filtered_records),
        "removed_cctv_embedding_records": len(removed),
        "active_source_ids": sorted(active_ids),
        "active_session_counts": active_counts,
        "active_source_kind_counts": active_kind_counts,
        "staged_sources_preserved_for_audit": True,
    ...
    updated_manifest = dict(staging_manifest)
    updated_manifest.update(
        {
            "pre_selection_embedding_usable_crops": int(
                staging_manifest.get("embedding_usable_crops", len(records))
            ),
            "embedding_usable_crops": len(filtered_records),
            "active_cctv_embedding_crops": len(filtered_records),
            "pre_selection_embedding_usable_session_counts": dict(
                staging_manifest.get("embedding_usable_session_counts") or {}
            ),
            "embedding_usable_session_counts": active_counts,
            "pre_selection_same_track_recovered_medoid_crops": int(
                staging_manifest.get("same_track_recovered_medoid_crops", 0)
            ),
            "same_track_recovered_medoid_crops": int(
                active_kind_counts.get("same_track_recovered_cctv_medoid", 0)
            ),
            "minimum_usable_evidence_passed": not missing_sessions,
            "missing_usable_sessions": missing_sessions,
            "source_selection": summary,
            "records": selected_staging.to_dict("records"),
        }
    )
    updated_manifest["minimum_usable_evidence_policy"] = {
        **dict(staging_manifest.get("minimum_usable_evidence_policy") or {}),
        "source_subset_selection_allowed": True,
        "source_subset_selection_policy": SOURCE_SELECTED_FAMILY_POLICY_VERSION,
        "staged_sources_preserved_for_audit": True,
        "final_promotion_requires_new_untouched_session": True,
    }
    ...
    return selected_staging, filtered_records, updated_manifest, summary, audit
    ...
    *,
    ...
    for historical in historical_source.sort_values("Relative_Path", kind="stable").to_dict(
        ...
        if review_action == ENROLLMENT_MULTIPLE:
            candidate_status = "excluded_reviewer_confirmed_multiple_people"
        elif review_action == ENROLLMENT_DUPLICATE:
            candidate_status = "excluded_reviewer_confirmed_duplicate"
        else:
            candidate_status = "candidate_owner_reconciled_to_target"
        item_rows.append(
            {
                "Correction_ID": "",
                "Source_Roll": source_roll,
...
def apply_identity_correction_to_approval_bundle(
    approval_bundle: CompactApprovalBundle,
    correction: IdentityCorrectionResult,
) -> CompactApprovalBundle:
    """Apply the proven folder-level correction to every affected reviewed source.

    The two explicit ``wrong_person_or_mislabeled`` rows are the human-confirmation
    anchor, while the historical/current hash reconciliation proves that the whole
    renamed folder belongs to AI0110. Therefore keep/duplicate/multiple-person
    decisions for the same proven hashes must follow the corrected owner too; they
    must not silently fall back to "deferred/unreviewed" merely because their review
    action was not ``wrong_person_or_mislabeled``.
    """

    source_roll = _canonical(correction.manifest.get("source_roll"))
    target_roll = _canonical(correction.manifest.get("target_roll"))
    if not source_roll or not target_roll or source_roll == target_roll:
        raise EmbeddingFamilyError("Identity correction manifest has invalid roll ownership")

    enrollment = approval_bundle.enrollment.copy()
    enrollment["Candidate_Roll_Before_Reconciliation"] = enrollment[
        "Candidate_Roll"
    ].map(_canonical)
    affected_mask = (
        enrollment["Expected_Roll"].map(_canonical).eq(source_roll)
        & enrollment["Source_SHA256"].map(_text).isin(correction.affected_hashes)
    )
    enrollment.loc[affected_mask, "Candidate_Roll"] = target_roll
    enrollment["Identity_Correction_Applied"] = affected_mask

    affected_rows = enrollment[affected_mask]
    if affected_rows.empty:
        raise EmbeddingFamilyError(
            "The validated folder correction did not match any enrollment review rows"
        )
    if not set(affected_rows["Candidate_Roll"].map(_canonical)) == {target_roll}:
        raise EmbeddingFamilyError("Affected enrollment review rows were not fully remapped")
    if set(
        enrollment.loc[~affected_mask, "Candidate_Roll"].map(_canonical)
    ) != set(
        enrollment.loc[~affected_mask, "Candidate_Roll_Before_Reconciliation"].map(
            _canonical
        )
    ):
        raise EmbeddingFamilyError("Identity correction changed an unrelated enrollment owner")

    cctv = approval_bundle.cctv.copy()
    cctv["Candidate_Roll_Before_Reconciliation"] = cctv["Candidate_Roll"].map(
        _canonical
    )
    cctv_mask = cctv["Item_ID"].map(_text).isin(correction.affected_cctv_item_ids)
    cctv.loc[cctv_mask, "Candidate_Roll"] = target_roll
    cctv["Identity_Correction_Applied"] = cctv_mask

    summary = dict(approval_bundle.summary)
    summary.update(
        {
            "identity_correction_applied": True,
            "identity_correction_id": _text(
                correction.manifest.get("correction_id")
            ),
            "reconciled_enrollment_review_rows": int(affected_mask.sum()),
            "reconciled_enrollment_review_item_ids": sorted(
                affected_rows["Item_ID"].map(_text)
            ),
            "reconciled_cctv_review_rows": int(cctv_mask.sum()),
            "reconciled_cctv_review_item_ids": sorted(
                cctv.loc[cctv_mask, "Item_ID"].map(_text)
            ),
            "global_alias_created": False,
        }
    )
    input_manifest = dict(approval_bundle.input_manifest)
    input_manifest["identity_correction"] = {
        "correction_id": _text(correction.manifest.get("correction_id")),
        "source_roll": source_roll,
        "target_roll": target_roll,
        "affected_enrollment_review_rows": int(affected_mask.sum()),
        "affected_cctv_review_rows": int(cctv_mask.sum()),
        "global_alias_created": False,
    }
    return CompactApprovalBundle(enrollment, cctv, summary, input_manifest)
...
def discover_enrollment_sources(
    *,
    ...
    for decision in approval_bundle.enrollment.to_dict("records"):
        ...
        if expected == SOURCE_IDENTITY and source_hash in correction.affected_hashes:
...
def _extract_single_cctv_embedding(
    image_path: Path,
    engine: FaceEngine,
    config: EnrollmentBuildConfig | None = None,
) -> tuple[np.ndarray, dict[str, Any]]:
    vector, metrics = _audit_single_cctv_embedding(image_path, engine, config)
    if vector is None:
        raise EmbeddingFamilyError(
            "Approved CCTV crop failed the production-compatible single-face "
            f"preprocessing policy: {image_path} "
            f"({metrics['embedding_exclusion_reason']}, "
            f"detections={metrics['detection_count']})"
        )
    return vector, metrics


def _audit_single_cctv_embedding(
    image_path: Path,
    engine: FaceEngine,
    config: EnrollmentBuildConfig | None = None,
) -> tuple[np.ndarray | None, dict[str, Any]]:
    """Audit one human-approved crop without weakening preprocessing.

    Human approval establishes identity provenance. This technical audit only decides
    whether the crop can safely contribute an SFace embedding. A crop that cannot pass
    the existing single-face/alignment policy remains preserved as human-approved
    evidence, but is excluded from the candidate embedding payload instead of aborting
    the entire family build.
    """

    effective_config = config or EnrollmentBuildConfig()
    extraction = extract_enrollment_embedding(
        Path(image_path),
        engine,
        effective_config,
    ...
    metrics: dict[str, Any] = {
        "width": int(extraction.image_width),
        "height": int(extraction.image_height),
        "detector_score": float(extraction.detector_score),
        "rank_score": float(extraction.rank_score),
        "detection_count": int(extraction.detection_count),
        "accepted_candidate_count": int(extraction.accepted_candidate_count),
        "preprocessing_policy": effective_config.validation_mode,
        "embedding_usable": False,
        "embedding_status": "approved_but_embedding_unusable",
        "embedding_exclusion_reason": _text(extraction.reason) or "unknown_preprocessing_failure",
    ...
    if not extraction.accepted or extraction.embedding is None:
        return None, metrics
    vector = normalize_embedding(extraction.embedding)
    if not vector.size or not np.isfinite(vector).all():
        metrics["embedding_exclusion_reason"] = "invalid_embedding"
        return None, metrics
    vector = vector.astype(np.float32)
    metrics.update(
        {
            "embedding_usable": True,
            "embedding_status": "embedding_usable",
            "embedding_exclusion_reason": "",
...
def stage_approved_cctv_sources(
    *,
    ...
    for ordinal, row in enumerate(approved.to_dict("records"), start=1):
        ...
        vector, metrics = _audit_single_cctv_embedding(destination, engine)
        ...
        rows.append(
            {
                ...
                "Human_Review_Action": CCTV_APPROVE,
                "Human_Approval_Preserved": True,
                "Embedding_Usable": bool(metrics["embedding_usable"]),
                "Embedding_Status": _text(metrics["embedding_status"]),
                "Embedding_Exclusion_Reason": _text(
                    metrics["embedding_exclusion_reason"]
                ),
                "Manual_Rereview_Required": False,
                "Quality_Gate_Relaxed": False,
                "Image_Width": int(metrics["width"]),
                "Image_Height": int(metrics["height"]),
                ...
                "Preprocessing_Policy": _text(metrics["preprocessing_policy"]),
                "Embedding_Dimension": int(metrics["embedding_dimension"]),
            }
        )
    ...
    staging = pd.DataFrame(rows)
    usable_staging = staging[staging["Embedding_Usable"].map(bool)]
    approved_session_counts = {
        _text(key): int(value)
        for key, value in staging["Source_Session"].value_counts().sort_index().items()
    }
    usable_session_counts = {
        _text(key): int(value)
        for key, value in usable_staging["Source_Session"]
        .value_counts()
        .sort_index()
        .items()
    }
    unusable_session_counts = {
        _text(key): int(value)
        for key, value in staging.loc[
            ~staging["Embedding_Usable"].map(bool), "Source_Session"
        ]
        .value_counts()
        .sort_index()
        .items()
    }
    missing_usable_sessions = [
        session
        for session in REQUIRED_CCTV_EMBEDDING_SESSIONS
        if int(usable_session_counts.get(session, 0)) < 1
    ]
    manifest = {
        "schema_version": FAMILY_SCHEMA_VERSION,
        "approved_rows": len(approved),
        "staged_unique_crops": len(staging),
        "embedding_usable_crops": len(usable_staging),
        "approved_but_embedding_unusable_crops": int(
            len(staging) - len(usable_staging)
        ),
        "rejected_rows_excluded": int(
            approval_bundle.cctv["Review_Action"].ne(CCTV_APPROVE).sum()
        ),
        "source_session_counts": approved_session_counts,
        "embedding_usable_session_counts": usable_session_counts,
        "embedding_unusable_session_counts": unusable_session_counts,
        "minimum_usable_evidence_policy": {
            "required_sessions": list(REQUIRED_CCTV_EMBEDDING_SESSIONS),
            "minimum_usable_crops_per_required_session": 1,
            "human_approval_is_not_embedding_usability": True,
            "unusable_approved_crop_action": "preserve_and_exclude_from_embedding_payload",
            "manual_rereview_required": False,
            "quality_gate_relaxed": False,
        },
        "minimum_usable_evidence_passed": not missing_usable_sessions,
        "missing_usable_sessions": missing_usable_sessions,
        "source_hashes_verified": True,
        "staged_hashes_verified": True,
        "human_actual_roll_controls_ownership": True,
        "model_prediction_controls_ownership": False,
        "all_human_approvals_preserved": True,
        "all_approved_crops_audited": len(staging) == len(approved),
        "manual_rereview_required": False,
        "quality_gate_relaxed": False,
        "live_dataset_writes": False,
        "super_resolution_used": False,
...
def _write_variant(
    *,
    ...
    (temporary_variant_dir / "build_report.md").write_text(
        "\n".join(
            [
                ...
                f"- Required CCTV source sessions: {', '.join(availability.get('required_source_sessions') or []) or 'none'}",
                f"- Missing technically usable source sessions: {', '.join(availability.get('missing_usable_source_sessions') or []) or 'none'}",
    ...
    manifest = {
        ...
        "required_source_sessions": list(
            availability.get("required_source_sessions") or ()
        ),
        "missing_usable_source_sessions": list(
            availability.get("missing_usable_source_sessions") or ()
        ),
        "adaptation_completeness": (
            "not_applicable"
            if not availability.get("required_source_sessions")
            else (
                "complete"
                if not availability.get("missing_usable_source_sessions")
                else "partial"
            )
        ),
...
def _write_unavailable_variant(
    *,
    temporary_variant_dir: Path,
    family_id: str,
    definition: VariantDefinition,
    ...
    """Write an auditable placeholder without fabricating an embedding variant."""
    variant_id = f"{family_id}-{definition.key.lower()}"
    temporary_variant_dir.mkdir(parents=True, exist_ok=False)
    unavailable_reason = _text(availability.get("unavailable_reason"))
    if not unavailable_reason:
        raise EmbeddingFamilyError(
            f"Unavailable variant {definition.key} is missing a deterministic reason"
        )
    report = {
        "schema_version": FAMILY_SCHEMA_VERSION,
        "version_id": variant_id,
        ...
        "variant_key": definition.key,
        "variant_role": definition.role,
        "status": VARIANT_UNAVAILABLE_STATUS,
        "availability": False,
        ...
        "evaluation_sessions_contaminated": [],
        "quality_gate_relaxed": False,
        "manual_rereview_required": False,
        "human_approval_invalidated": False,
        "embedding_payload_fabricated": False,
        "production_promoted": False,
        "mon_p3_processed": False,
    }
    _write_json(temporary_variant_dir / "unavailable_report.json", report)
    (temporary_variant_dir / "build_report.md").write_text(
        "\n".join(
            [
                f"# {variant_id}",
                "",
                f"Status: `{VARIANT_UNAVAILABLE_STATUS}`",
                "",
                f"- Role: `{definition.role}`",
                f"- Reason: `{unavailable_reason}`",
                "- Embedding payload created: no",
                "- Quality gate relaxed: no",
                "- Manual re-review required: no",
                "- Production promoted: no",
                "- MON_P3 processed: no",
                "",
            ]
    ...
    manifest = {
        ...
        "embedding_records": 0,
        "embedding_dimension": expected_dimension,
        "per_student_image_counts": {roll: 0 for roll in sorted(roster_rolls)},
        "per_student_embedding_counts": {roll: 0 for roll in sorted(roster_rolls)},
        "canonical_identity_coverage": {
            "covered": 0,
            "roster": len(roster_rolls),
            "rolls": [],
        },
        "missing_students": sorted(roster_rolls),
        "required_source_sessions": list(
            availability.get("required_source_sessions") or ()
        ),
        "missing_usable_source_sessions": list(
        ...
        "source_sessions_included": [],
        "source_sessions_excluded": [TUE_P1_SESSION, TUE_P2_SESSION],
        "evaluation_sessions_allowed": [],
        "evaluation_sessions_contaminated": [],
        "training_contaminated_descriptive_only": definition.descriptive_only,
        "adaptation_completeness": "unavailable",
        "artifact_sha256": _variant_artifact_hashes(temporary_variant_dir),
        "quality_gate_relaxed": False,
        "manual_rereview_required": False,
        "embedding_payload_fabricated": False,
...
def apply_family_safety_gates(
    *,
    ...
    variant_availability: dict[str, dict[str, Any]] | None = None,
    benchmark_expected_rows: int = 120,
    expected_tue_p1_accepted_rows: int | None = 37,
) -> tuple[dict[str, Any], pd.DataFrame]:
    availability = variant_availability or {
        key: {
            "available": True,
            "status": VARIANT_BUILT_STATUS,
            "missing_usable_source_sessions": [],
        }
        for key in ("A", "B", "C", "D")
    }
    safe_specs = {
    ...
    unsafe_model_rows: set[tuple[str, str]] = set()
    unsafe_benchmark_rows: set[str] = set()
    ...
    unavailable_variants = [
        key
        for key in ("A", "B", "C", "D")
        if not bool(dict(availability.get(key) or {}).get("available"))
    ]
    ...
    missing_baseline_columns = sorted(required_prediction_columns.difference(baseline.columns))
    if missing_baseline_columns:
        raise EmbeddingFamilyError(
            "Production benchmark predictions are missing columns: "
            + ", ".join(missing_baseline_columns)
        )
    if baseline["Benchmark_Row_ID"].duplicated().any():
        structure_failures.append("Production baseline contains duplicate benchmark row IDs")
    if len(baseline) != benchmark_expected_rows:
        structure_failures.append(
            f"Production baseline row count changed: expected {benchmark_expected_rows}, "
            f"found {len(baseline)}"
        )
    expected_sessions_by_variant = {
        "A": {TUE_P1_SESSION, TUE_P2_SESSION},
        "B": {TUE_P1_SESSION},
        "C": {TUE_P2_SESSION},
        "D": {TUE_P1_SESSION, TUE_P2_SESSION},
    }
    for key, expected_sessions in expected_sessions_by_variant.items():
        available = bool(dict(availability.get(key) or {}).get("available"))
        if key not in variant_predictions:
            if available:
                structure_failures.append(f"Variant {key} predictions are missing")
            continue
        frame = variant_predictions[key]
        if not available:
            if len(frame):
                structure_failures.append(
                    f"Unavailable variant {key} unexpectedly produced predictions"
                )
            continue
        missing_columns = sorted(required_prediction_columns.difference(frame.columns))
        if missing_columns:
            structure_failures.append(
                f"Variant {key} predictions are missing columns: {', '.join(missing_columns)}"
            )
            continue
        if frame["Benchmark_Row_ID"].duplicated().any():
            structure_failures.append(
                f"Variant {key} predictions contain duplicate benchmark row IDs"
            )
        expected_ids = set(
            baseline.loc[
                baseline["Session_ID"].isin(expected_sessions), "Benchmark_Row_ID"
            ].map(_text)
        )
        actual_ids = set(frame["Benchmark_Row_ID"].map(_text))
        if actual_ids != expected_ids:
            structure_failures.append(
                f"Variant {key} benchmark row set changed: "
                f"missing={sorted(expected_ids - actual_ids)[:5]}, "
                f"extra={sorted(actual_ids - expected_ids)[:5]}"
            )
        actual_sessions = set(frame["Session_ID"].map(_text))
        if actual_sessions != expected_sessions:
            structure_failures.append(
                f"Variant {key} prediction sessions changed: "
                f"expected={sorted(expected_sessions)}, found={sorted(actual_sessions)}"
            )
    if expected_tue_p1_accepted_rows is not None:
        accepted_population = baseline[
            baseline["Source_Kind"].eq("tue_p1_accepted_review")
        ]
        if len(accepted_population) != expected_tue_p1_accepted_rows:
            structure_failures.append(
                "TUE_P1 accepted-review population changed: "
                f"expected {expected_tue_p1_accepted_rows}, found {len(accepted_population)}"
            )

    for key, sessions in safe_specs.items():
        if key not in variant_predictions or not bool(
            dict(availability.get(key) or {}).get("available")
        ):
            continue
        ...
        unsafe_event_mask = (
            ...
            | comparison["Correct_To_Wrong_Regression"].map(bool)
            | comparison["Outsider_Absorption_Candidate"].map(bool)
            | comparison["Mixed_Acceptance_Candidate"].map(bool)
        )
        unsafe_ids = sorted(
            comparison.loc[unsafe_event_mask, "Benchmark_Row_ID"].map(_text).unique()
        )
        metrics["unsafe_model_rows"] = int(unsafe_event_mask.sum())
        metrics["unsafe_benchmark_row_ids"] = unsafe_ids
        comparison_metrics[key] = metrics
        for benchmark_row_id in unsafe_ids:
            unsafe_model_rows.add((key, benchmark_row_id))
            unsafe_benchmark_rows.add(benchmark_row_id)
    ...
    structure_passed = not structure_failures
    leakage_passed = not leakage_failures
    false_identity_passed = not unsafe_model_rows
    b_available = bool(dict(availability.get("B") or {}).get("available"))
    decisive_retention = retention_comparisons.get("B", {}) if b_available else {}
    fallback_retention = retention_comparisons.get("A", {})
    lost_correct_acceptances = int(
        decisive_retention.get("lost_correct_acceptances", 0)
    )
    fallback_lost_correct_acceptances = int(
        fallback_retention.get("lost_correct_acceptances", 0)
    )
    retention_evaluable = bool(b_available and decisive_retention)
    retention_passed = (
        lost_correct_acceptances == 0
        if retention_evaluable
        else fallback_lost_correct_acceptances == 0
    )
    coverage_passed = candidate_coverage >= production_roster_coverage
    benchmark_evidence_sufficient = feature_rows == benchmark_expected_rows
    cross_session_pair_complete = bool(
        dict(availability.get("B") or {}).get("available")
        and dict(availability.get("C") or {}).get("available")
    )
    evidence_sufficient = bool(
        benchmark_evidence_sufficient and cross_session_pair_complete
    )
    ...
    gates = {
        ...
        "evaluation_structure": {
            ...
            "expected_benchmark_rows": benchmark_expected_rows,
            "production_baseline_rows": len(baseline),
            "expected_tue_p1_accepted_rows": expected_tue_p1_accepted_rows,
            "exact_row_sets_required": True,
        },
        "reviewed_accepted_track_retention": {
            ...
            "decisive_variant": "B" if retention_evaluable else None,
            "fallback_diagnostic_variant": "A" if not retention_evaluable else None,
            "decisive_retention_evaluable": retention_evaluable,
            "unavailable_reason": (
                _text(dict(availability.get("B") or {}).get("unavailable_reason"))
                if not retention_evaluable
                else ""
            ),
            "lost_correct_acceptances": lost_correct_acceptances,
            "fallback_lost_correct_acceptances": fallback_lost_correct_acceptances,
            "comparisons": retention_comparisons,
        },
        "zero_new_false_accepts": {
            "passed": false_identity_passed,
            "unsafe_model_rows": len(unsafe_model_rows),
            "unique_unsafe_benchmark_rows": len(unsafe_benchmark_rows),
            "unsafe_benchmark_row_ids": sorted(unsafe_benchmark_rows),
            "policy": (
                "zero new known-student wrong accepts; zero candidate not-in-mapping "
                "absorptions; zero candidate mixed-track accepts"
            ),
        ...
        "evidence_sufficiency": {
            ...
            "benchmark_feature_evidence_passed": benchmark_evidence_sufficient,
            "cross_session_pair_complete": cross_session_pair_complete,
            "unavailable_variants": unavailable_variants,
            "missing_usable_source_sessions": {
                key: list(
                    dict(availability.get(key) or {}).get(
                        "missing_usable_source_sessions"
                    )
                    or []
                )
                for key in unavailable_variants
            },
            "evidence_mode": "reviewer_contact_sheet_reextraction",
        },
    }
    if (
        ...
        or not approval_integrity
        or not correction_integrity
        or not structure_passed
        or not leakage_passed
    ):
    ...
    elif not benchmark_evidence_sufficient:
    ...
    elif not cross_session_pair_complete:
        decision = "built_unapproved_insufficient_evidence"
        next_step = (
            "Preserve this partial family and do not repeat the completed manual reviews. "
            "Recover technically usable TUE_P2 adaptation evidence through an automated, "
            "same-track best-frame workflow or future live exception evidence, then build a "
            "new immutable family. MON_P3 remains untouched and blocked."
        )
    else:
    ...
    result = {
        ...
        "production_approved": False,
        "candidate_promoted": False,
        "all_available_variants_built_unapproved": True,
        "unavailable_variants": unavailable_variants,
        "match_threshold": OFFICIAL_MATCH_THRESHOLD,
        "margin_threshold": OFFICIAL_MARGIN_THRESHOLD,
        "aggregate_mode": OFFICIAL_AGGREGATE,
...
def run_family_evaluation(
    *,
    ...
    variant_availability: dict[str, dict[str, Any]],
    engine: FaceEngine,
    roster_rolls: set[str],
    production_roster_coverage: int,
    protected_comparison: dict[str, Any],
    model_hashes: dict[str, str],
    source_selected_family: bool = False,
    progress: Callable[[str], None] | None = None,
    ...
    for key in ("A", "B", "C", "D"):
        ...
        available = bool(dict(variant_availability.get(key) or {}).get("available"))
        if not available:
            variant_predictions[key] = baseline.head(0).copy()
            continue
        ...
        variant_predictions[key] = score_benchmark(
            ...
            allowed_sessions=tuple(manifest.get("evaluation_sessions_allowed") or ()),
            training_contaminated_descriptive=bool(
                manifest.get("training_contaminated_descriptive_only")
            ),
        )
    ...
    _write_json(
        evaluation_dir / "variant_availability.json",
        {
            "schema_version": FAMILY_SCHEMA_VERSION,
            "family_id": family_id,
            "variants": {
                key: dict(variant_availability.get(key) or {})
                for key in ("A", "B", "C", "D")
            },
            "cross_session_pair_complete": bool(
                dict(variant_availability.get("B") or {}).get("available")
                and dict(variant_availability.get("C") or {}).get("available")
            ),
            "manual_rereview_required": False,
            "quality_gate_relaxed": False,
        },
    ...
    if source_selected_family:
        if _text(decision.get("decision")) != "built_unapproved_pending_mon_p3":
            raise EmbeddingFamilyError(
                "Source-selected family did not pass the frozen regression gates required "
                "for a new untouched-session candidate"
            )
        decision.update(
            {
                "decision": "built_unapproved_pending_new_untouched_session",
                "status": "built_unapproved_pending_new_untouched_session",
                "exact_next_step": (
                    "Reserve and run a different untouched validation session. MON_P3 is "
                    "design evidence and cannot be reused as final promotion evidence."
                ),
                "mon_p3_reuse_for_promotion_allowed": False,
                "new_untouched_session_required": True,
                "candidate_source_selection_applied": True,
            }
        )
    ...
    safe_parts = [
        variant_predictions[key]
        for key in ("B", "C")
        if bool(dict(variant_availability.get(key) or {}).get("available"))
    ]
    safe_combined = (
        pd.concat(safe_parts, ignore_index=True).sort_values(
            "Benchmark_Row_ID", kind="stable"
        )
        if safe_parts
        else baseline.head(0).copy()
    )
    expected_safe_ids = set()
    if bool(dict(variant_availability.get("B") or {}).get("available")):
        expected_safe_ids.update(
            benchmark.loc[
                benchmark["Session_ID"].eq(TUE_P1_SESSION), "Benchmark_Row_ID"
            ].map(_text)
        )
    if bool(dict(variant_availability.get("C") or {}).get("available")):
        expected_safe_ids.update(
            benchmark.loc[
                benchmark["Session_ID"].eq(TUE_P2_SESSION), "Benchmark_Row_ID"
            ].map(_text)
        )
    actual_safe_ids = set(safe_combined["Benchmark_Row_ID"].map(_text))
    if (
        safe_combined["Benchmark_Row_ID"].duplicated().any()
        or actual_safe_ids != expected_safe_ids
    ):
        raise EmbeddingFamilyError(
            "Available cross-session predictions do not match their exact leakage-safe row set"
        )
    _write_csv(
    ...
    predictions_for_summary = {
        ...
        "available_leakage_safe_cross_session_combined": safe_combined,
    }
    _write_csv(
    ...
    for key in ("A", "B", "C", "D"):
        manifest = variant_manifests[key]
        coverage = dict(manifest.get("canonical_identity_coverage") or {})
        coverage_rows.append(
            {
                "Model_ID": manifest["version_id"],
                "Available": bool(manifest.get("availability", True)),
                "Status": _text(manifest.get("status")),
                "CVO_Covered": int(coverage.get("covered", 0)),
                "CVO_Roster": int(coverage.get("roster", len(roster_rolls))),
                "Missing_Rolls": ";".join(manifest.get("missing_students") or []),
    ...
    regression_summary = {
        ...
        "available_cross_session_rows": len(safe_combined),
        "cross_session_pair_complete": bool(
            dict(variant_availability.get("B") or {}).get("available")
            and dict(variant_availability.get("C") or {}).get("available")
        ),
        ...
        "production_approved": False,
        "candidate_promoted": False,
        "variant_d_label": "training-contaminated descriptive result over actual usable CCTV sessions",
    ...
    for name in report_order:
        value = metrics[name]
        key = {
            "variant_a_cleaned_enrollment": "A",
            "variant_b_tue_p1_cross_session": "B",
            "variant_c_tue_p2_cross_session": "C",
            "variant_d_full_contaminated_descriptive": "D",
        }.get(name)
        availability_label = (
            "available"
            if key is None or bool(dict(variant_availability.get(key) or {}).get("available"))
            else VARIANT_UNAVAILABLE_STATUS
        )
        table_rows.append(
            f"| {name} | {availability_label} | {value['rows']} | {value['accepted']} | "
    ...
    (evaluation_dir / "regression_report.md").write_text(
        "\n".join(
            [
                ...
                *table_rows,
                "",
                "Variant D is a **training-contaminated descriptive result over the CCTV sessions "
                "that produced technically usable embeddings**. It is not promotion evidence. "
                "Unavailable cross-session variants are recorded explicitly and are never "
                "fabricated from enrollment-only records.",
                "",
                "## Safety status",
                "",
...
def _approval_output_frame(frame: pd.DataFrame, *, enrollment: bool) -> pd.DataFrame:
    ...
    preferred = (
        [
            ...
            "Reviewer_Notes",
            "Expected_Roll",
            "Candidate_Roll_Before_Reconciliation",
            "Candidate_Roll",
...
def _student_source_coverage(
    *,
    ...
    for roll in sorted(roster_rolls):
        ...
        usable_cctv_group = cctv_group[cctv_group["Embedding_Usable"].map(bool)]
        row = {
            "Canonical_Roll": roll,
            ...
            "Embedding_Usable_CCTV_TUE_P1": int(
                usable_cctv_group["Source_Session"].eq(TUE_P1_SESSION).sum()
            ),
            "Embedding_Usable_CCTV_TUE_P2": int(
                usable_cctv_group["Source_Session"].eq(TUE_P2_SESSION).sum()
            ),
            "Approved_But_Embedding_Unusable_CCTV": int(
                (~cctv_group["Embedding_Usable"].map(bool)).sum()
            ),
...
def preflight_embedding_family_inputs(
    *,
    repo_root: Path,
    forensic_run_dir: Path,
    enrollment_review_package: Path,
    ...
    """Run every read-only Phase 1.2I-B integrity check before model work.

    This intentionally stops before face extraction, candidate staging, version creation,
    or benchmark rescoring. It is safe to run repeatedly and provides the launcher with a
    fail-closed gate before the expensive one-time build.
    """
    ...
    versions_root = Path(
        versions_root or repo_root / "models" / "versions"
    ).resolve()
    recovery_dir = Path(recovery_dir).resolve() if recovery_dir else None
    historical_inventory_path = Path(
    ...
    production = _ensure_production_preflight(
        production_embeddings_path=production_embeddings_path,
        production_summary_path=production_summary_path,
    ...
    approvals = apply_identity_correction_to_approval_bundle(approvals, correction)
    ...
    _roster, _names, subject_rolls_raw = load_student_mapping(student_map_path, "CVO")
    roster_rolls = {
        _canonical(value) for value in subject_rolls_raw if _canonical(value)
    }
    if len(roster_rolls) != 27:
        raise EmbeddingFamilyError(
            f"Authoritative CVO roster must contain 27 students, found {len(roster_rolls)}"
        )
    if TARGET_IDENTITY not in roster_rolls or SOURCE_IDENTITY in roster_rolls:
        raise EmbeddingFamilyError(
            "Authoritative CVO roster does not preserve the AI0110/CSE0110 distinction"
        )

    recovery_plan: RecoveryConsumptionPlan | None = None
    if recovery_dir is not None:
        recovery_plan = validate_same_track_recovery_for_family(
            recovery_dir=recovery_dir,
    ...
    protected_comparison = compare_protected_states(protected_before, protected_after)
    if not protected_comparison["unchanged"]:
        raise EmbeddingFamilyError(
            "Protected inputs changed during read-only preflight: "
            f"files={protected_comparison['changed_fixed_files'][:5]}, "
            f"trees={protected_comparison['changed_trees']}"
        )

    corrected_review_rows = approvals.enrollment[
        approvals.enrollment["Identity_Correction_Applied"].map(bool)
    ]
    approved_cctv = approvals.cctv[
        approvals.cctv["Review_Action"].eq(CCTV_APPROVE)
    ]
    result = {
        "schema_version": FAMILY_SCHEMA_VERSION,
        "policy_version": (
            RECOVERY_FAMILY_POLICY_VERSION if recovery_plan is not None else POLICY_VERSION
        ),
        "status": "passed",
        "read_only": True,
        "candidate_build_started": False,
        "face_extraction_started": False,
        "production": production,
        "approval_validation": approvals.summary,
        "identity_correction": {
            "correction_id": correction.manifest["correction_id"],
            "affected_image_hashes": len(correction.affected_hashes),
            "reconciled_review_rows": len(corrected_review_rows),
            "explicit_human_confirmation_rows": len(
                correction.affected_enrollment_item_ids
            ),
            "affected_cctv_rows": len(correction.affected_cctv_item_ids),
            "global_alias_created": False,
        },
        "shruthi_source": shruthi,
        "cvo_roster_count": len(roster_rolls),
        "ai0110_in_cvo": TARGET_IDENTITY in roster_rolls,
        "cse0110_in_cvo": SOURCE_IDENTITY in roster_rolls,
        "configured_enrollment_images": len(discovered),
        "policy_eligible_before_face_extraction": int(
            discovered["Policy_Eligible"].map(bool).sum()
        ),
        "reviewer_confirmed_duplicates_excluded": int(
            discovered["Policy_Reason"].eq("reviewer_confirmed_duplicate").sum()
        ),
        "reviewer_multiple_people_excluded": int(
            discovered["Policy_Reason"].eq("reviewer_confirmed_multiple_people").sum()
        ),
        "approved_cctv_rows": len(approved_cctv),
        "approved_cctv_session_counts": {
            _text(key): int(value)
            for key, value in approved_cctv["Source_Session"]
            .value_counts()
            .sort_index()
            .items()
        },
        "frozen_benchmark": benchmark_manifest,
        ...
        "official_attendance_changed": False,
        "mon_p3_processed": False,
        "review_scalability_policy": _review_scalability_policy(),
        "same_track_recovery_consumption": (
            {
                key: value
                for key, value in recovery_plan.summary.items()
                if key != "recovery_dir"
            }
            if recovery_plan is not None
            else None
        ),
        "manual_reviews_repeated": False,
        "safe_to_start_candidate_build": True,
    }
    result["preflight_fingerprint_sha256"] = _stable_digest(
        {key: value for key, value in result.items() if key != "preflight_fingerprint_sha256"}
    )
    return result
...
def build_and_evaluate_embedding_family(
    *,
    ...
    recovery_dir: Path | None = None,
    cctv_source_selection: CctvSourceSelectionContract | None = None,
    ...
    if recovery_dir is not None:
        if progress:
            progress("Immutable same-track recovery validation and medoid consumption planning")
        recovery_plan = validate_same_track_recovery_for_family(
            recovery_dir=recovery_dir,
            versions_root=versions_root,
            production_embeddings_path=production_embeddings_path,
    ...
    input_fingerprint = {
        "policy_version": (
            SOURCE_SELECTED_FAMILY_POLICY_VERSION
            if cctv_source_selection is not None
            else (
        ...
        "build_configuration": asdict(EnrollmentBuildConfig()),
        "cctv_embedding_usability_policy": {
            "human_approval_is_not_embedding_usability": True,
            "single_face_preprocessing_remains_required": True,
            "unusable_approved_crop_action": "preserve_and_exclude_from_embedding_payload",
            "required_sessions": list(REQUIRED_CCTV_EMBEDDING_SESSIONS),
            "minimum_usable_crops_per_required_session": 1,
            "missing_session_action": "mark_dependent_variant_unavailable_and_continue_partial_family",
            "cross_session_pair_required_for_mon_p3_advance": True,
            "unavailable_variant_payload_fabricated": False,
            "manual_rereview_required": False,
            "quality_gate_relaxed": False,
            "super_resolution_used": False,
        },
        ...
        "cctv_source_selection": (
            cctv_source_selection.normalized()
            if cctv_source_selection is not None
            else None
        ),
    }
    ...
    try:
        ...
        recovery_consumption_manifest: dict[str, Any] | None = None
        if recovery_plan is not None:
            if progress:
                progress("Staging one immutable recovered medoid per reviewed TUE_P2 track")
            recovery_staging, recovery_records, recovery_consumption_manifest = (
            ...
            cctv_staging, cctv_records, staging_manifest = (
                merge_recovery_into_cctv_staging(
                    original_staging=cctv_staging,
                    original_records=cctv_records,
                    original_manifest=staging_manifest,
                    recovery_staging=recovery_staging,
                    recovery_records=recovery_records,
                    recovery_manifest=recovery_consumption_manifest,
                )
            )
        source_selection_summary: dict[str, Any] | None = None
        source_selection_audit: pd.DataFrame | None = None
        if cctv_source_selection is not None:
            if recovery_plan is None:
                raise EmbeddingFamilyError(
                    "A source-selected candidate family requires the immutable recovery bundle"
                )
            if progress:
                progress("Applying immutable Phase 1.2L.1 CCTV source selection")
            (
                cctv_staging,
                cctv_records,
                staging_manifest,
                ...
                staging=cctv_staging,
                records=cctv_records,
                staging_manifest=staging_manifest,
                contract=cctv_source_selection,
            )
            _write_json(
                temporary_family_dir / "candidate_source_selection.json",
                source_selection_summary,
            )
            _write_csv(
                temporary_family_dir / "candidate_source_selection_audit.csv",
                source_selection_audit,
            )
        _write_csv(
            temporary_family_dir / "cctv_embedding_usability.csv", cctv_staging
        )
        _write_json(
            temporary_family_dir / "candidate_staging_manifest.json", staging_manifest
        )
        variant_availability = resolve_variant_availability(staging_manifest)
        _write_json(
            temporary_family_dir / "variant_availability_plan.json",
            variant_availability,
        )
        ...
        candidate_source_manifest = {
            ...
            "approved_cctv_source_count": int(
                staging_manifest.get("approved_rows", expected_approved_cctv)
            ),
            "same_track_recovered_medoid_source_count": int(
                staging_manifest.get("same_track_recovered_medoid_crops", 0)
            ),
            "usable_cctv_embedding_source_count": len(cctv_records),
            "approved_but_embedding_unusable_source_count": int(
                staging_manifest.get("approved_but_embedding_unusable_crops", 0)
            ),
            "enrollment_sources": [
                {
                    "source_id": _text(row["Source_ID"]),
            ...
            "active_cctv_embedding_source_ids": [
                _text(record["_source_id"]) for record in cctv_records
            ],
            "variant_availability": variant_availability,
            "same_track_recovery_consumption": recovery_consumption_manifest,
            "candidate_source_selection": source_selection_summary,
            "source_hashes_verified": True,
        ...
        variant_directories: dict[str, Path] = {}
        if progress:
            progress("Writing available immutable variants and explicit unavailable placeholders")
        for base_definition in VARIANT_DEFINITIONS:
            availability = dict(variant_availability[base_definition.key])
            definition = _effective_variant_definition(
                base_definition, availability
            )
            ...
            common_kwargs = {
                ...
                "parent_summary_sha256": production_preflight[
                    "production_summary"
                ]["sha256"],
                "model_hashes": model_hashes,
                "source_manifest_sha256": source_manifest_sha256,
                "enrollment_approval_sha256": _sha256_file(
            ...
            if availability["available"]:
                ...
                unavailable_kwargs = dict(common_kwargs)
                unavailable_kwargs.pop("availability")
                manifest = _write_unavailable_variant(
                    availability=availability,
                    **unavailable_kwargs,
                )
        ...
        decision, regression_summary = run_family_evaluation(
            ...
            protected_comparison=protected_comparison,
            model_hashes=model_hashes,
            source_selected_family=cctv_source_selection is not None,
            progress=progress,
        )
        variant_summary = {
            key: {
                ...
                "availability": bool(manifest.get("availability", True)),
                "unavailable_reason": _text(manifest.get("unavailable_reason")),
                "embedding_records": manifest["embedding_records"],
                "embedding_dimension": manifest["embedding_dimension"],
                "embedding_sha256": (
                    _sha256_file(variant_directories[key] / "student_embeddings.pkl")
                    if bool(manifest.get("availability", True))
                    else None
                ),
                "summary_sha256": (
                    _sha256_file(variant_directories[key] / "embedding_summary.csv")
                    if bool(manifest.get("availability", True))
                    else None
                ),
                ...
                "required_source_sessions": manifest.get(
                    "required_source_sessions", []
                ),
                "missing_usable_source_sessions": manifest.get(
                    "missing_usable_source_sessions", []
                ),
        ...
        family_manifest = {
            ...
            "approved_cctv_staged_count": int(
                staging_manifest.get("approved_rows", expected_approved_cctv)
            ),
            "same_track_recovered_medoid_count": int(
                staging_manifest.get("same_track_recovered_medoid_crops", 0)
            ),
            "usable_cctv_embedding_count": len(cctv_records),
            "approved_but_embedding_unusable_count": int(
                staging_manifest.get("approved_but_embedding_unusable_crops", 0)
            ),
            "same_track_recovery_consumption": recovery_consumption_manifest,
            "candidate_source_selection": source_selection_summary,
            "variant_availability": variant_availability,
            "cctv_embedding_usability_policy": staging_manifest[
                "minimum_usable_evidence_policy"
            ],
            ...
            "new_untouched_session_required": bool(
                cctv_source_selection is not None
            ),
            "candidate_built": bool(
                dict(variant_availability.get("D") or {}).get("available")
            ),
            "candidate_promoted": False,
            "production_approved": False,
        ...
        if launcher_source and Path(launcher_source).is_file():
            launcher_name = (
                "run_phase_1_2m_candidate_family.ps1"
                if cctv_source_selection is not None
                else "run_phase_1_2i_b.ps1"
            )
            shutil.copy2(launcher_source, temporary_family_dir / launcher_name)
        output_manifest_path = _write_output_manifest(
            ...
            extra={
                ...
                "production_approved": False,
                "mon_p3_processed": False,
                "candidate_source_selection_applied": bool(
                    cctv_source_selection is not None
                ),
                "new_untouched_session_required": bool(
                    cctv_source_selection is not None
        ...
        source_to_preserve = (
            final_family_dir
            if finalized_to_family_id and final_family_dir.exists()
            else temporary_family_dir
        )
        preserved_path = source_to_preserve
        try:
            if source_to_preserve.exists():
                _write_json(
                    source_to_preserve / "failure_report.json",
                    {
                        "schema_version": FAMILY_SCHEMA_VERSION,
                        "family_id": resolved_family_id,
                        "status": "build_failed_preserved",
                        "failed_at": _now(),
                        "failed_after_final_directory_move": finalized_to_family_id,
                ...
                if preserved_path.exists():
                    preserved_path = versions_root / (
                        f"{failed_name}-{time.time_ns()}"
                    )
                source_to_preserve.replace(preserved_path)
        except OSError:
            pass
        raise EmbeddingFamilyError(
...
def verify_embedding_family(
    ...
    *,
    ...
    recovery_consumption = family_manifest.get("same_track_recovery_consumption")
    if recovery_consumption is not None:
        recovery_consumption = dict(recovery_consumption)
        if (
            _text(recovery_consumption.get("policy_version"))
            != RECOVERY_FAMILY_POLICY_VERSION
            or int(recovery_consumption.get("candidate_embedding_record_count", -1)) != 5
            or int(recovery_consumption.get("staged_source_count", -1)) != 5
            or recovery_consumption.get("human_review_labels_reused") is not True
            or recovery_consumption.get("model_prediction_used_as_identity") is not False
            or recovery_consumption.get("manual_rereview_required") is not False
            or recovery_consumption.get("quality_gate_relaxed") is not False
            or recovery_consumption.get("super_resolution_used") is not False
            or recovery_consumption.get("candidate_promoted") is not False
            or recovery_consumption.get("mon_p3_processed") is not False
        ):
            raise EmbeddingFamilyError(
                "Recovered-family consumption manifest violates its safety contract"
            )
        if not (family_dir / "recovery_consumption_manifest.json").is_file():
            raise EmbeddingFamilyError(
                "Recovered family is missing its recovery-consumption manifest"
            )
        if not (family_dir / "recovery_consumption_sources.csv").is_file():
            raise EmbeddingFamilyError(
                "Recovered family is missing its recovery-consumption source audit"
            )
    source_selection = family_manifest.get("candidate_source_selection")
    normalized_selection: dict[str, Any] | None = None
    if source_selection is not None:
        normalized_selection = dict(source_selection)
        required_selection_files = (
            family_dir / "candidate_source_selection.json",
            family_dir / "candidate_source_selection_audit.csv",
        )
        if not all(path.is_file() for path in required_selection_files):
            raise EmbeddingFamilyError(
                "Source-selected family is missing its immutable selection artifacts"
            )
        recorded_selection = json.loads(
            required_selection_files[0].read_text(encoding="utf-8")
        )
        if recorded_selection != normalized_selection:
            raise EmbeddingFamilyError(
                "Family and recorded CCTV source-selection contracts differ"
            )
        kept = set(normalized_selection.get("kept_source_ids") or [])
        removed = set(normalized_selection.get("removed_source_ids") or [])
        implicated = set(normalized_selection.get("implicated_source_ids") or [])
        if (
            _text(normalized_selection.get("policy_version"))
            != SOURCE_SELECTED_FAMILY_POLICY_VERSION
            or not kept
            or not removed
            or kept & removed
            or implicated != kept | removed
            or normalized_selection.get(
                "final_promotion_requires_new_untouched_session"
            )
            is not True
            or normalized_selection.get("candidate_promoted") is not False
            or family_manifest.get("new_untouched_session_required") is not True
            or family_manifest.get("mon_p3_reuse_for_promotion_allowed") is not False
            or output_manifest.get("candidate_source_selection_applied") is not True
            or output_manifest.get("new_untouched_session_required") is not True
        ):
            raise EmbeddingFamilyError(
                "Source-selected family violates its selection or validation-session contract"
            )
    variants = dict(family_manifest.get("variants") or {})
    definitions = {definition.key: definition for definition in VARIANT_DEFINITIONS}
    verified_unavailable: list[str] = []
    variant_source_ids: dict[str, set[str]] = {}
    variant_source_kinds: dict[str, dict[str, str]] = {}
    for key, definition in definitions.items():
        ...
        available = bool(variant_manifest.get("availability", True))
        expected_status = (
            VARIANT_BUILT_STATUS if available else VARIANT_UNAVAILABLE_STATUS
        )
        if (
            variant_manifest.get("status") != expected_status
        ...
        contaminated = set(
            variant_manifest.get("evaluation_sessions_contaminated") or []
        )
        required = set(variant_manifest.get("required_source_sessions") or [])
        missing = set(
            variant_manifest.get("missing_usable_source_sessions") or []
        )
        expected_required = set(definition.included_cctv_sessions)
        if required != expected_required:
            raise EmbeddingFamilyError(
                f"Variant {key} required-source contract changed: {sorted(required)}"
            )
        if not available:
            if key == "A":
                raise EmbeddingFamilyError("Variant A may not be unavailable")
            if not _text(variant_manifest.get("unavailable_reason")):
                raise EmbeddingFamilyError(
                    f"Unavailable variant {key} has no reason"
                )
            if included or allowed or contaminated:
                raise EmbeddingFamilyError(
                    f"Unavailable variant {key} claims source or evaluation sessions"
                )
            if missing != required:
                raise EmbeddingFamilyError(
                    f"Unavailable variant {key} has an invalid missing-source contract"
                )
            if (variant_dir / "student_embeddings.pkl").exists():
                raise EmbeddingFamilyError(
                    f"Unavailable variant {key} fabricated an embedding payload"
                )
            if int(variant_manifest.get("embedding_records", -1)) != 0:
                raise EmbeddingFamilyError(
                    f"Unavailable variant {key} records a nonzero embedding count"
                )
            if variant_manifest.get("embedding_payload_fabricated") is not False:
                raise EmbeddingFamilyError(
                    f"Unavailable variant {key} does not preserve the no-fabrication contract"
                )
            if variant_manifest.get("promotion_allowed_in_phase_1_2i_b") is not False:
                raise EmbeddingFamilyError(
                    f"Unavailable variant {key} permits unsafe phase promotion"
                )
            verified_unavailable.append(key)
            continue
        ...
        source_records_path = variant_dir / "source_records.csv"
        source_records = pd.read_csv(
            source_records_path, dtype=str, keep_default_na=False
        )
        if (
            len(source_records) != len(records)
            or "Source_ID" not in source_records.columns
            or source_records["Source_ID"].duplicated().any()
        ):
            raise EmbeddingFamilyError(
                f"Variant {key} source-record audit does not match its embedding payload"
            )
        variant_source_ids[key] = set(source_records["Source_ID"].map(_text))
        variant_source_kinds[key] = {
            _text(row["Source_ID"]): _text(row.get("Source_Kind"))
            for row in source_records.to_dict("records")
        }
        if included & allowed:
            raise EmbeddingFamilyError(f"Variant {key} has an evaluation leakage overlap")
        if key == "A" and included:
            raise EmbeddingFamilyError(
                "Variant A unexpectedly includes CCTV adaptation sessions"
            )
        if key == "B" and included != {TUE_P2_SESSION}:
            raise EmbeddingFamilyError(
                f"Variant B included-session contract changed: {sorted(included)}"
            )
        if key == "C" and included != {TUE_P1_SESSION}:
            raise EmbeddingFamilyError(
                f"Variant C included-session contract changed: {sorted(included)}"
            )
        if key == "D" and (
            not included
            or not included.issubset({TUE_P1_SESSION, TUE_P2_SESSION})
        ):
            raise EmbeddingFamilyError(
                f"Variant D included-session contract changed: {sorted(included)}"
            )
        if missing != required - included:
            raise EmbeddingFamilyError(
                f"Variant {key} missing-source contract changed: {sorted(missing)}"
            )
        ...
        if _text(variant_manifest.get("adaptation_completeness")) != expected_completeness:
            raise EmbeddingFamilyError(
                f"Variant {key} adaptation completeness is inconsistent"
            )
        expected_allowed = {
            "A": {TUE_P1_SESSION, TUE_P2_SESSION},
            "B": {TUE_P1_SESSION},
        ...
        if allowed != expected_allowed:
            raise EmbeddingFamilyError(
                f"Variant {key} evaluation-session contract changed: {sorted(allowed)}"
            )
        expected_contaminated = set() if key == "A" else included
        if contaminated != expected_contaminated:
            raise EmbeddingFamilyError(
                f"Variant {key} contaminated-session contract changed: "
                f"{sorted(contaminated)}"
            )
        if variant_manifest.get("training_contaminated_descriptive_only") is not (
            definition.descriptive_only
        ):
            raise EmbeddingFamilyError(
                f"Variant {key} descriptive-only contract changed"
            )
        if variant_manifest.get("promotion_allowed_in_phase_1_2i_b") is not False:
            raise EmbeddingFamilyError(f"Variant {key} permits unsafe phase promotion")
    if normalized_selection is not None:
        kept = set(normalized_selection["kept_source_ids"])
        removed = set(normalized_selection["removed_source_ids"])
        full_ids = variant_source_ids.get("D", set())
        if not kept.issubset(full_ids) or removed.intersection(full_ids):
            raise EmbeddingFamilyError(
                "Full candidate does not match the immutable CCTV source selection"
            )
        for source_id in removed:
            for key in ("B", "C", "D"):
                if source_id in variant_source_ids.get(key, set()):
                    raise EmbeddingFamilyError(
                        f"Removed source {source_id} remains active in Variant {key}"
                    )
        cctv_ids = {
            source_id
            for source_id, kind in variant_source_kinds.get("D", {}).items()
            if kind != "cleaned_enrollment"
        }
        candidate_source_manifest = json.loads(
            (family_dir / "candidate_source_manifest.json").read_text(encoding="utf-8")
        )
        recorded_active = set(
            candidate_source_manifest.get("active_cctv_embedding_source_ids") or []
        )
        if recorded_active != cctv_ids:
            raise EmbeddingFamilyError(
                "Candidate source manifest active CCTV IDs differ from Variant D"
            )
        if dict(candidate_source_manifest.get("candidate_source_selection") or {}) != normalized_selection:
            raise EmbeddingFamilyError(
                "Candidate source manifest selection contract changed"
            )
        selection_audit = pd.read_csv(
            family_dir / "candidate_source_selection_audit.csv",
            dtype=str,
            keep_default_na=False,
        )
        if (
            "Source_ID" not in selection_audit.columns
            or "Selected_For_Candidate" not in selection_audit.columns
            or selection_audit["Source_ID"].duplicated().any()
        ):
            raise EmbeddingFamilyError(
                "Candidate source-selection audit schema is invalid"
            )
        audit_selected = set(
            selection_audit.loc[
                selection_audit["Selected_For_Candidate"].map(_yes), "Source_ID"
            ].map(_text)
        )
        if audit_selected != cctv_ids:
            raise EmbeddingFamilyError(
                "Candidate source-selection audit active IDs differ from Variant D"
            )
    ...
    try:
        ...
        raise EmbeddingFamilyError(str(exc)) from exc
    if recovery_consumption is not None:
        if not bool(dict(variants.get("B") or {}).get("availability")):
            raise EmbeddingFamilyError(
                "Recovered family did not make leakage-safe Variant B available"
            )
        if int(dict(variants.get("B") or {}).get("embedding_records", 0)) <= int(
            dict(variants.get("A") or {}).get("embedding_records", 0)
        ):
            raise EmbeddingFamilyError(
                "Recovered family Variant B did not consume any TUE_P2 medoid evidence"
            )
    decision = json.loads(
    ...
    allowed_decisions = {
        "regression_failed_false_identity",
        "regression_failed_integrity",
        "hold_coverage_or_retention_regression",
        "built_unapproved_pending_mon_p3",
        "built_unapproved_pending_new_untouched_session",
        "built_unapproved_insufficient_evidence",
    }
    if _text(decision.get("decision")) not in allowed_decisions:
        raise EmbeddingFamilyError("Evaluation decision is not an allowed family state")
    if normalized_selection is not None and (
        _text(decision.get("decision"))
        != "built_unapproved_pending_new_untouched_session"
        or decision.get("new_untouched_session_required") is not True
        or decision.get("mon_p3_reuse_for_promotion_allowed") is not False
    ):
        raise EmbeddingFamilyError(
            "Source-selected family does not require a new untouched validation session"
        )
    if decision.get("mon_p3_processed") is not False:
        raise EmbeddingFamilyError("Evaluation decision claims MON_P3 was processed")
    decision_unavailable = sorted(
        _text(value) for value in decision.get("unavailable_variants") or []
    )
    if decision_unavailable != sorted(verified_unavailable):
        raise EmbeddingFamilyError(
            "Evaluation decision unavailable-variant set does not match the family"
        )
