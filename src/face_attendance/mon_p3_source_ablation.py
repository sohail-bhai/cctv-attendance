...
EXPECTED_SOURCE_ABLATION_ID = "source-ablation-da12b41f85bc813a6de1"
EXPECTED_SOURCE_ABLATION_MANIFEST_SHA256 = "2a34c5c107e4966fdfafed8a2a2e5d5e2bae1e5ab2a914116c97eacad82c54a3"
EXPECTED_LEGACY_RECOMMENDED_CONFIG_ID = "ABL-19-f659a1f5"
EXPECTED_CORRECTED_RECOMMENDED_CONFIG_ID = "ABL-16-ee3a4ff6"
...
def rank_configurations(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    result = frame.sort_values(
        ...
        # Build selection must be decided only by hard gates, independent-session
        # evidence and parsimony. Same-session descriptive evidence is report-only
        # and must never outrank a safer composition with fewer added sources.
...
def _default_source_ablation_output(repo_root: Path) -> Path:
    return (
        Path(repo_root).resolve()
        / "attendance_output"
        / "embedding_forensics"
        / "phase_1_2l"
        / EXPECTED_SOURCE_ABLATION_ID
    )


def inspect_source_ablation_selection(
    *,
    repo_root: Path,
    ...
    repo_root = Path(repo_root).resolve()
    source_output = (
        Path(source_output).resolve()
        if source_output is not None
        else _default_source_ablation_output(repo_root)
    )
    if source_output.name != EXPECTED_SOURCE_ABLATION_ID:
        raise SourceAblationError(
            "Phase 1.2L.1 must consume the exact completed source-ablation output "
            f"{EXPECTED_SOURCE_ABLATION_ID}"
        )
    verified = verify_source_ablation_output(source_output)
    source_manifest = source_output / "output_manifest.json"
    source_manifest_sha256 = _sha256_file(source_manifest)
    if source_manifest_sha256 != EXPECTED_SOURCE_ABLATION_MANIFEST_SHA256:
        raise SourceAblationError(
            "Phase 1.2L output manifest hash differs from the reviewed evidence bundle"
        )

    production_embeddings = repo_root / "models" / "student_embeddings.pkl"
    production_summary = repo_root / "models" / "embedding_summary.csv"
    if _sha256_file(production_embeddings) != EXPECTED_PRODUCTION_EMBEDDINGS_SHA256:
        raise SourceAblationError("Protected production embeddings changed")
    if _sha256_file(production_summary) != EXPECTED_PRODUCTION_SUMMARY_SHA256:
        raise SourceAblationError("Protected production summary changed")

    experiments = pd.read_csv(source_output / "subset_experiment_summary.csv")
    if len(experiments) != EXPECTED_SUBSET_COUNT:
        raise SourceAblationError("Phase 1.2L experiment summary is incomplete")
    experiments["Hard_Gates_Passed"] = _as_bool_series(
        experiments["Hard_Gates_Passed"]
    )
    corrected_ranking = rank_configurations(experiments.drop(columns=["Rank"]))
    hard_gate_candidates = corrected_ranking[
        corrected_ranking["Hard_Gates_Passed"].map(bool)
    ].copy()
    if hard_gate_candidates.empty:
        raise SourceAblationError("Phase 1.2L has no hard-gate candidate")
    frontier = _pareto_frontier(hard_gate_candidates)
    corrected = hard_gate_candidates.iloc[0].to_dict()

    legacy_payload = _load_json(
        source_output / "recommended_composition.json",
        "legacy Phase 1.2L recommendation",
    )
    legacy = dict(legacy_payload.get("recommended_configuration") or {})
    legacy_id = _text(legacy.get("Config_ID"))
    corrected_id = _text(corrected.get("Config_ID"))
    if legacy_id != EXPECTED_LEGACY_RECOMMENDED_CONFIG_ID:
        raise SourceAblationError(
            "Legacy Phase 1.2L recommendation is not the reviewed ABL-19 result"
        )
    if corrected_id != EXPECTED_CORRECTED_RECOMMENDED_CONFIG_ID:
        raise SourceAblationError(
            "Promotion-evidence-first selection did not resolve to the expected ABL-16 composition"
        )
    if corrected_id not in set(frontier["Config_ID"].astype(str)):
        raise SourceAblationError(
            "Corrected recommendation is not on the safe candidate frontier"
        )
    ...
    promotion_metrics = [
        ...
        "Benchmark_Safe_Correct_Accepted",
        "MON_P3_Unsafe_Accepts",
        "MON_P3_Lost_Correct_Production_Accepts",
        "Benchmark_Safe_Unsafe_Accepts",
        "Benchmark_Safe_Unverifiable_Accepts",
        "Benchmark_Safe_Lost_Correct_Production_Accepts",
    ]
    mismatched_metrics = [
        key for key in promotion_metrics if int(corrected[key]) != int(legacy[key])
    ]
    if mismatched_metrics:
        raise SourceAblationError(
            "Corrected and legacy configurations differ on promotion-eligible metrics: "
            + ", ".join(mismatched_metrics)
        )
    if int(corrected["Kept_Source_Count"]) >= int(legacy["Kept_Source_Count"]):
        raise SourceAblationError(
            "Corrected recommendation is not more parsimonious than the legacy recommendation"
        )

    descriptive = pd.read_csv(
        source_output / "benchmark_descriptive_subset_predictions.csv"
    )
    legacy_predictions = descriptive[
        descriptive["Model_ID"].astype(str) == legacy_id
    ].copy()
    corrected_predictions = descriptive[
        descriptive["Model_ID"].astype(str) == corrected_id
    ].copy()
    if len(legacy_predictions) != EXPECTED_BENCHMARK_ROWS or len(
        corrected_predictions
    ) != EXPECTED_BENCHMARK_ROWS:
        raise SourceAblationError(
            "Descriptive benchmark predictions are incomplete for selection audit"
        )
    joined = legacy_predictions.merge(
        corrected_predictions,
        on="Benchmark_Row_ID",
        suffixes=("_Legacy", "_Corrected"),
        validate="one_to_one",
    )
    legacy_correct = _as_bool_series(joined["Correct_Accepted_Legacy"])
    corrected_correct = _as_bool_series(joined["Correct_Accepted_Corrected"])
    descriptive_only = joined[legacy_correct & ~corrected_correct].copy()
    expected_descriptive_delta = int(
        legacy["Benchmark_Descriptive_Correct_Accepted"]
    ) - int(corrected["Benchmark_Descriptive_Correct_Accepted"])
    if len(descriptive_only) != expected_descriptive_delta or len(descriptive_only) != 2:
        raise SourceAblationError(
            "Legacy descriptive advantage does not reproduce as exactly two rows"
        )
    promotion_eligible = _as_bool_series(
        descriptive_only["Promotion_Evidence_Eligible_Legacy"]
    )
    if promotion_eligible.any():
        raise SourceAblationError(
            "Legacy-only descriptive gains unexpectedly contain promotion-eligible evidence"
        )
    if set(descriptive_only["Session_ID_Legacy"].astype(str)) != {TUE_P1_SESSION}:
        raise SourceAblationError(
            "Legacy-only descriptive gains are not confined to the TUE_P1 source session"
        )
    if set(descriptive_only["Actual_Roll_Legacy"].astype(str)) != {"2401100CSE0121"}:
        raise SourceAblationError(
            "Legacy-only descriptive gains are not confined to student 2401100CSE0121"
        )

    fingerprint = _stable_digest(
        {
            "selection_policy_version": SELECTION_POLICY_VERSION,
            "source_ablation_id": EXPECTED_SOURCE_ABLATION_ID,
            "source_manifest_sha256": source_manifest_sha256,
            "legacy_config_id": legacy_id,
            "corrected_config_id": corrected_id,
        }
    )
    audit_id = f"selection-audit-{fingerprint[:20]}"
    output_dir = (
        repo_root
        / "attendance_output"
        / "embedding_forensics"
    ...
    return {
        ...
        "source_output_verified_files": int(verified.get("verified_files", 0)),
        "source_manifest_sha256": source_manifest_sha256,
        "legacy": legacy,
        "corrected": corrected,
...
def verify_source_ablation_selection_audit(output_dir: Path) -> dict[str, Any]:
    output_dir = Path(output_dir).resolve()
    try:
        verified = verify_output_manifest(output_dir, output_dir / "output_manifest.json")
    except ShadowValidationError as exc:
        raise SourceAblationError(str(exc)) from exc
    decision = _load_json(
        output_dir / "selection_decision.json",
        "Phase 1.2L.1 selection decision",
    )
    if _text(decision.get("selection_policy_version")) != SELECTION_POLICY_VERSION:
        raise SourceAblationError("Phase 1.2L.1 selection policy changed")
    if _text(decision.get("source_ablation_id")) != EXPECTED_SOURCE_ABLATION_ID:
        raise SourceAblationError("Phase 1.2L.1 source-ablation provenance changed")
    if _text(decision.get("legacy_recommended_config_id")) != EXPECTED_LEGACY_RECOMMENDED_CONFIG_ID:
        raise SourceAblationError("Phase 1.2L.1 legacy recommendation changed")
    if _text(decision.get("corrected_recommended_config_id")) != EXPECTED_CORRECTED_RECOMMENDED_CONFIG_ID:
        raise SourceAblationError("Phase 1.2L.1 corrected recommendation changed")
    if decision.get("corrected_config_is_on_safe_frontier") is not True:
        raise SourceAblationError("Corrected selection is not recorded on the safe frontier")
    for key in (
        "production_embeddings_changed",
        "official_attendance_changed",
        "candidate_family_built",
        "candidate_promoted",
        "full_mon_p3_recognition_repeated",
        "manual_review_repeated",
    ):
        if decision.get(key) is not False:
            raise SourceAblationError(f"Selection audit safety field changed: {key}")
    return {
        "status": "PASS",
        "output_dir": str(output_dir),
        "verified_files": int(verified.get("verified_files", 0)),
        "audit_id": _text(decision.get("audit_id")),
        "legacy_recommended_config_id": EXPECTED_LEGACY_RECOMMENDED_CONFIG_ID,
        "corrected_recommended_config_id": EXPECTED_CORRECTED_RECOMMENDED_CONFIG_ID,
        "production_embeddings_changed": False,
        "official_attendance_changed": False,
        "candidate_family_built": False,
        "candidate_promoted": False,
...
def create_source_ablation_selection_audit(
    *,
    repo_root: Path,
    source_output: Path | None = None,
    ...
    output_dir = Path(inspected["output_dir"])
    if output_dir.exists():
        return verify_source_ablation_selection_audit(output_dir), output_dir, True
    ...
    if temporary.exists():
        ...
        os.replace(temporary, quarantine)
        if status:
            status(f"Preserved prior incomplete selection audit: {quarantine}")
    temporary.mkdir(parents=True, exist_ok=False)

    corrected_ranking = inspected["corrected_ranking"].copy()
    corrected_ranking.to_csv(temporary / "selection_candidates.csv", index=False)
    descriptive_only = inspected["descriptive_only"].copy()
    descriptive_columns = [
        "Benchmark_Row_ID",
        "Session_ID_Legacy",
        "Tracklet_ID_Legacy",
        "Actual_Roll_Legacy",
        "Promotion_Evidence_Eligible_Legacy",
        "Accepted_Legacy",
        "Best_Roll_Legacy",
        "Best_Score_Legacy",
        "Margin_Legacy",
        "Accepted_Corrected",
        "Best_Roll_Corrected",
        "Best_Score_Corrected",
        "Margin_Corrected",
    ]
    descriptive_only[descriptive_columns].to_csv(
        temporary / "descriptive_only_deltas.csv", index=False
    )
    ...
    decision = {
        ...
        "selection_policy_version": SELECTION_POLICY_VERSION,
        "created_at": datetime.now().isoformat(timespec="seconds"),
        "audit_id": inspected["audit_id"],
        "decision": "legacy_recommendation_superseded_by_promotion_evidence_parsimony",
        "source_ablation_id": EXPECTED_SOURCE_ABLATION_ID,
        "source_ablation_manifest_sha256": inspected["source_manifest_sha256"],
        "legacy_recommended_config_id": _text(legacy.get("Config_ID")),
        "corrected_recommended_config_id": _text(corrected.get("Config_ID")),
        "corrected_config_is_on_safe_frontier": True,
        "legacy_kept_source_count": int(legacy["Kept_Source_Count"]),
        "corrected_kept_source_count": int(corrected["Kept_Source_Count"]),
        "corrected_kept_source_labels": _text(corrected.get("Kept_Source_Labels")),
        "corrected_kept_source_ids": _text(corrected.get("Kept_Source_IDs")),
        "corrected_removed_source_ids": _text(corrected.get("Removed_Source_IDs")),
        "promotion_eligible_metrics_equal_to_legacy": True,
        "legacy_only_descriptive_rows": int(len(descriptive_only)),
        "legacy_only_descriptive_rows_promotion_eligible": 0,
        "mon_p3_correct_accepted": int(corrected["MON_P3_Correct_Accepted"]),
        "mon_p3_unsafe_accepts": int(corrected["MON_P3_Unsafe_Accepts"]),
        "mon_p3_lost_correct_production_accepts": int(
            corrected["MON_P3_Lost_Correct_Production_Accepts"]
        ),
        "mon_p3_recovered_correct_vs_production": int(
            corrected["MON_P3_Recovered_Correct_vs_Production"]
        ),
        "benchmark_safe_correct_accepted": int(
            corrected["Benchmark_Safe_Correct_Accepted"]
        ),
        "benchmark_safe_unsafe_accepts": int(
            corrected["Benchmark_Safe_Unsafe_Accepts"]
        ),
        "benchmark_safe_lost_correct_production_accepts": int(
            corrected["Benchmark_Safe_Lost_Correct_Production_Accepts"]
        ),
        "benchmark_safe_recovered_correct_vs_production": int(
            corrected["Benchmark_Safe_Recovered_Correct_vs_Production"]
        ),
        "selection_is_build_design_only": True,
        "final_promotion_requires_new_untouched_session": True,
        "production_embeddings_changed": False,
        ...
        "manual_review_repeated": False,
        "exact_next_step": (
            "Build a new immutable candidate family that retains only the CP1 TUE_P2 "
            "recovered medoid for 2401100CSE0140 from the five implicated sources, then "
            "run frozen regression and reserve a new untouched validation session."
        ),
    }
    (temporary / "selection_decision.json").write_text(
        json.dumps(decision, indent=2, ensure_ascii=True), encoding="utf-8"
    )
    report = [
        "# Phase 1.2L.1 Promotion-Evidence Selection Audit",
        "",
        f"- Source ablation: `{EXPECTED_SOURCE_ABLATION_ID}`",
        f"- Legacy recommendation: `{legacy['Config_ID']}` with {legacy['Kept_Source_Count']} implicated sources",
        f"- Corrected recommendation: `{corrected['Config_ID']}` with {corrected['Kept_Source_Count']} implicated source",
        f"- Corrected kept source: {corrected['Kept_Source_Labels']}",
        "- MON_P3 recoveries preserved: 4/4",
        "- MON_P3 correct production accepts lost: 0",
        "- Leakage-safe benchmark correct production accepts lost: 0",
        "- Unsafe accepts: 0",
        "",
        "The legacy ABL-19 recommendation and corrected ABL-16 recommendation are equal on all promotion-eligible metrics. ABL-19 was ranked first only because two TUE_P1 same-session rows improved in descriptive mode, where Promotion_Evidence_Eligible is false. The corrected policy therefore selects the frontier configuration with fewer added sources.",
        "",
        "This audit does not build or promote a family and does not modify production, attendance, datasets, recognition outputs or completed reviews.",
    ]
    (temporary / "selection_report.md").write_text(
        "\n".join(report) + "\n", encoding="utf-8"
    )
    write_output_manifest(
        temporary,
    ...
    return verify_source_ablation_selection_audit(output_dir), output_dir, False
