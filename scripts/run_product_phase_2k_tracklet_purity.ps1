...
$runner = Join-Path $PSScriptRoot "run_product_phase_2k_tracklet_purity.py"

if (-not (Test-Path -LiteralPath $python -PathType Leaf)) {
    throw "Project virtual-environment Python is missing: $python"
}
if (-not (Test-Path -LiteralPath $runner -PathType Leaf)) {
    throw "Phase 2K-A Python runner is missing: $runner"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Product Phase 2K-A failed closed with exit code $LASTEXITCODE"
}
