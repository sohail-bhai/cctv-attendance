...
def parse_args() -> argparse.Namespace:
    ...
    family_parser.add_argument(
        ...
        default="",
        help=(
            "Optional immutable Phase 1.2I-C same-track recovery bundle. When "
            "provided, the family consumes exactly one validated medoid per reviewed "
            "TUE_P2 track and remains unapproved."
        ),
    )
    ...
    family_preflight_parser.add_argument(
        ...
        default="",
        help=(
            "Optional immutable same-track recovery bundle to validate before "
            "the recovered-family build."
        ),
    )
    family_preflight_parser.add_argument(
        "--output-json",
...
def main() -> None:
    ...
    try:
        ...
        if args.command == "preflight-embedding-family":
            ...
            if result.get("same_track_recovery_consumption"):
                recovery = result["same_track_recovery_consumption"]
                print(f"Recovery ID: {recovery['recovery_id']}")
                print(
                    "Recovery consumption: "
                    f"{recovery['candidate_consumed_medoid_embeddings']} medoids from "
                    f"{recovery['target_tracks']} reviewed tracks"
                )
                print("Manual reviews repeated: no")
        ...
        if args.command == "build-evaluate-embedding-family":
            recovered_build = bool(args.recovery_dir)
            print(
                "Phase 1.2I-D recovered version-family build"
                if recovered_build
                else "Phase 1.2I-B version-family build"
            )
            print(f"Repository: {ROOT}")
            print(f"Frozen benchmark: {Path(args.forensic_run).resolve()}")
            print(f"Enrollment approvals: {Path(args.enrollment_approvals).resolve()}")
            ...
            if recovered_build:
                print(f"Recovery bundle: {Path(args.recovery_dir).resolve()}")
                print("Recovery consumption: one validated medoid per human-reviewed track")
            ...
            result = build_and_evaluate_embedding_family(
                ...
                launcher_source=(
                    ROOT / "scripts" / "run_phase_1_2i_d_recovered_family.ps1"
                    if recovered_build
                    else ROOT / "scripts" / "run_phase_1_2i_b.ps1"
                ),
                recovery_dir=(Path(args.recovery_dir) if recovered_build else None),
            )
